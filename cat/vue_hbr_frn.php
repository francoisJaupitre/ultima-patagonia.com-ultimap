...
?>
				class="lnk"
				onclick="window.parent.opn_frm('cat/ctr.php?cbl=frn&id=<?php echo $id_frn ?>');"
<?php
}
?>
			>
...
?>
				type="text"
				style="width: 100%;"
				value="<?php echo stripslashes($mel_hbr) ?>"
				onChange="updateData('cat_hbr', 'mail', this.value, <?php echo $id ?>)"
			/>
		</td>
	</tr>
	<tr>
...
?>
				type="text"
				style="width: 100%;"
				value="<?php echo stripslashes($ctc_hbr) ?>"
				onChange="updateData('cat_hbr', 'contact', this.value, <?php echo $id ?>)"
			/>
		</td>
	</tr>
	<tr>
...
?>
				type="text"
				style="width: 100%;"
				value="<?php echo stripslashes($tel_hbr) ?>"
				onChange="updateData('cat_hbr', 'tel_res', this.value, <?php echo $id ?>)"
			/>
		</td>
	</tr>
	<tr>
		<td id="hbr_wap" class="fwb"><?php include("vue_hbr_wap.php"); ?></td>
		<td>
			<input
<?php
if(!$aut['cat'] or $id_frn)
...
?>
				type="text"
				style="width: 100%;"
				value="<?php if($wap_hbr > 0){echo $wap_hbr;} ?>"
				onChange="updateData('cat_hbr', 'wap', this.value, <?php echo $id ?>)"
			/>
		</td>
	</tr>
	<tr>
...
?>
				style="height: 100px;"
				onChange="updateData('cat_hbr', 'bnq', this.value, <?php echo $id ?>)"
			>
<?php
echo stripslashes($bnq_hbr)
?>
			</textarea>
		</td>
	</tr>
	<tr>
		...
		<td>
			<div class="wsn">
				<input
<?php
if(!$aut['adm_fin'] or $id_frn)
{
	echo ' disabled';
}
?>
 					id="hbr_frs<?php echo $id ?>"
					type="text"
					style="width: 35px;"
					value="<?php echo $frs_hbr*100 ?>"
					onChange="updateData('cat_hbr', 'frs', this.value, <?php echo $id ?>)"
				/>%
				<input
					type="text"
<?php
...
?>
					value="<?php echo stripslashes($notfrs_hbr) ?>"
					onChange="updateData('cat_hbr', 'notfrs', this.value, <?php echo $id ?>)"
				/>
			</div>
		</td>
	</tr>
