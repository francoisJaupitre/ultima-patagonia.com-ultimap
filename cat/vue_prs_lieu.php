...
?>
	<tr>
		<td><input <?php if(!$aut['cat']) {echo ' disabled';} ?> type="number" class="w25" value="<?php echo $dt_lieu['ord']; ?>" onchange="maj('cat_prs_lieu','ord',this.value,<?php echo $dt_lieu['id'].','.$id ?>)" /></td>
		...
		<td><input <?php if(!$aut['cat']) {echo ' disabled';} ?> type="checkbox" autocomplete="off" <?php if ($dt_lieu['shw']) {echo('checked="checked"');} ?> onclick="if(this.checked) {maj('cat_prs_lieu','shw','1',<?php echo  $dt_lieu['id'] ?>)}else{maj('cat_prs_lieu','shw','0',<?php echo  $dt_lieu['id'] ?>)};" /></td>
<?php
	if($aut['cat']) {
?>
