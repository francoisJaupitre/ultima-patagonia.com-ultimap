...
?>
		<tr>
			...
			<td><input <?php if(!$aut['cmp']) {echo ' disabled';} ?> type="checkbox" autocomplete="off" <?php if($dt_pay['pay']) {echo('checked="checked"');} ?> onclick="if(this.checked) {maj('dev_srv_pay','pay','1',<?php echo $dt_pay['id_srv_pay'] ?>)}else{maj('dev_srv_pay','pay','0',<?php echo $dt_pay['id_srv_pay'] ?>)};" /></td>
		</tr>
<?php
}
?>
...
?>
		<tr>
			<td><input type="text" <?php if(!$aut['cat']) {echo ' disabled';} ?> class="w25" value="<?php echo $dt_pay['taux']*100 ?>" onChange="maj('cat_frn_pay','taux',this.value,<?php echo $dt_pay['id'] ?>)" />%</td>
			<td><input type="number" <?php if(!$aut['cat']) {echo ' disabled';} ?> class="w25" value="<?php echo $dt_pay['delai'] ?>" onChange="maj('cat_frn_pay','delai',this.value,<?php echo $dt_pay['id'] ?>)" /><?php echo $txt->jours->$id_lng ?></td>
