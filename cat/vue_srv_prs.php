<?php
$nb_prs = ftc_ass(select("COUNT(*) as total","cat_prs_srv","id_srv",$id));
if($nb_prs['total']!=0) {
?>
<div class="lmcf fwb"><?php echo $txt->prss->$id_lng.' :'; ?></div>
<table style=" width:100%;" class="dsg">
<?php
	$rq_prs = select("cat_prs.id,nom,opt,info","cat_prs INNER JOIN cat_prs_srv ON cat_prs.id = cat_prs_srv.id_prs","id_srv",$id,"opt DESC,nom","DISTINCT");
