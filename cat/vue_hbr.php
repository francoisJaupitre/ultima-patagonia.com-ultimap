<?php
$dt_hbr = ftc_ass(sel_quo("*", "cat_hbr", "id", $id));
$vrl = $dt_hbr['vrl'];
...
?>
												...
												id="nom_hbr"
												style="width: 200px;"
												value="<?php echo stripslashes($dt_hbr['nom']) ?>"
												onChange="updateData('cat_hbr', 'nom', this.value, <?php echo $id ?>);"
											/>
										</div>
									</td>
								</tr>
...
?>
																type="text"
																class="w-100"
																value="<?php echo stripslashes($mail_frt_hbr) ?>"
																onChange="updateData('cat_hbr', 'mail_frt', this.value, <?php echo $id ?>)"
															/>
														</td>
													</tr>
													<tr>
...
?>
															 ...
															 maxlength="50"
															 value="<?php echo stripslashes($dt_hbr['info']) ?>"
															 onChange="updateData('cat_hbr', 'info', this.value, <?php echo $id ?>)"
															/>
														</td>
													</tr>
													<tr>
...
?>
																type="text"
																class="w-100"
																value="<?php echo stripslashes($dt_hbr['alerte']) ?>"
																onChange="updateData('cat_hbr', 'alerte', this.value, <?php echo $id ?>)"
															/>
														</td>
													</tr>
													<tr>
...
?>
															 ...
															 onclick="
															 	if(this.checked)
																{
																	updateData('cat_hbr', 'lstrg', '1', <?php echo $id ?>)
																}else{
																	updateData('cat_hbr', 'lstrg', '0', <?php echo $id ?>)
																}
															"
															/>
														</td>
...
?>
																...
																onclick="
																	if(this.checked)
																	{
																		updateData('cat_hbr', 'ferme', '1', <?php echo $id ?>)
																	}else{
																		updateData('cat_hbr', 'ferme', '0', <?php echo $id ?>)
																	};
																"
															/>
														</td>
...
?>
													...
													onChange="updateData('cat_hbr', 'notes', this.value, <?php echo $id ?>)"
												>
<?php
echo stripslashes($dt_hbr['notes'])
?>
												</textarea>
											</div>
										</div>
...
?>
										 type="text"
										 class="w-100"
										 value="<?php echo stripslashes($dt_hbr['tel_hbr']) ?>"
										 onChange="updateData('cat_hbr', 'tel_hbr', this.value, <?php echo $id ?>)"
										/>
									</td>
...
?>
											...
											class="w-100"
											style="min-width: 200px;"
											value="<?php echo stripslashes($dt_hbr['adresse']) ?>"
											onChange="updateData('cat_hbr', 'adresse', this.value, <?php echo $id ?>)"
										/>
									</td>
...
?>
											type="time"
											value="<?php if(!is_null($dt_hbr['chk_in'])) {echo date("H:i", strtotime($dt_hbr['chk_in']));} ?>"
											onblur="updateData('cat_hbr', 'chk_in', this.value, <?php echo $id ?>)"
										/>
										<strong>/</strong>
										<input
<?php
...
?>
										 type="time"
										 value="<?php if(!is_null($dt_hbr['chk_out'])) {echo date("H:i", strtotime($dt_hbr['chk_out']));} ?>"
										 onblur="updateData('cat_hbr', 'chk_out', this.value, <?php echo $id ?>)"
										/>
									</td>
								</tr>
							</table>
...
?>
			 type="text"
			 id="hbr_lat<?php echo $id ?>"
			 style="width: 60px"
			 value="<?php echo $dt_hbr['lat']; ?>"
			 onChange="updateData('cat_hbr', 'lat', this.value, <?php echo $id.', 0' ?>);"
			/>(N+/S-) /
			<input
<?php
if(!$aut['cat'])
...
?>
			 type="text"
			 id="hbr_lon<?php echo $id ?>"
			 style="width: 60px"
			 value="<?php echo $dt_hbr['lon']; ?>"
			 onChange="updateData('cat_hbr', 'lon', this.value, <?php echo $id.', 0' ?>);"
			/>(E+/W-)
		</div>
		<div id="map" class="w-100" style="height: 220px;"></div>
	</div>
