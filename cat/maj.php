...
if((is_numeric($car) or $car == '(' or $car == ')' or $car == '=' or $car == '+' or $car == '-') and !in_array($col,$not_col)) {
	...
	if($flg)
	{
		...
		else{echo $txt->errval->$id_lng;return;}
		if($col == 'frs' || $col =='taux') {$val /= 100;}
...
if($col=="ord") {
	...
	if($val>0) {
		if($tab=="cat_crc_mdl") {
			...
			if($val>$max[0]) {$flg_col = false;}
			else{
				$rq_crc_mdl = select("id,ord","cat_crc_mdl","id_crc",$id_crc);
				while($dt_crc_mdl = ftc_ass($rq_crc_mdl)) {
					if($dt_crc_mdl['ord'] > $ord_init and $dt_crc_mdl['ord'] <= $val) {upd_noq("cat_crc_mdl","ord","ord-1",$dt_crc_mdl['id']);}
					if($dt_crc_mdl['ord'] < $ord_init and $dt_crc_mdl['ord'] >= $val) {upd_noq("cat_crc_mdl","ord","ord+1",$dt_crc_mdl['id']);}
				}
			}
		}
		elseif($tab=="cat_mdl_jrn") {
			...
			if($val>$max[0]) {$flg_col = false;}
				...
				while($dt_mdl_jrn = ftc_ass($rq_mdl_jrn)) {
					if($dt_mdl_jrn['ord'] > $ord_init and $dt_mdl_jrn['ord'] <= $val) {upd_noq("cat_mdl_jrn","ord","ord-1",$dt_mdl_jrn['id']);}
					if($dt_mdl_jrn['ord'] < $ord_init and $dt_mdl_jrn['ord'] >= $val) {upd_noq("cat_mdl_jrn","ord","ord+1",$dt_mdl_jrn['id']);}
				}
			}
		}
		elseif($tab=="cat_jrn_prs") {
			...
			if($val>$max[0]) {$flg_col = false;}
				...
				while($dt_jrn_prs = ftc_ass($rq_jrn_prs)) {
					if($dt_jrn_prs['ord'] > $ord_init and $dt_jrn_prs['ord'] <= $val) {upd_noq("cat_jrn_prs","ord","ord-1",$dt_jrn_prs['id']);}
					if($dt_jrn_prs['ord'] < $ord_init and $dt_jrn_prs['ord'] >= $val) {upd_noq("cat_jrn_prs","ord","ord+1",$dt_jrn_prs['id']);}
					if($dt_jrn_prs['ord'] == $ord_init and $dt_jrn_prs['id']!= $id) {upd_quo("cat_jrn_prs","ord",$val,$dt_jrn_prs['id']);}
				}
			}
		}
		elseif($tab=="cat_jrn_vll") {
			...
			if($val>$max[0]) {$flg_col = false;}
				...
				while($dt_jrn_vll = ftc_ass($rq_jrn_vll)) {
					if($dt_jrn_vll['ord'] > $ord_init and $dt_jrn_vll['ord'] <= $val) {upd_noq("cat_jrn_vll","ord","ord-1",$dt_jrn_vll['id']);}
					if($dt_jrn_vll['ord'] < $ord_init and $dt_jrn_vll['ord'] >= $val) {upd_noq("cat_jrn_vll","ord","ord+1",$dt_jrn_vll['id']);}
				}
			}
		}
		elseif($tab=="cat_jrn_lieu") {
			...
			if($val>$max[0]) {$flg_col = false;}
				...
				while($dt_jrn_lieu = ftc_ass($rq_jrn_lieu)) {
					if($dt_jrn_lieu['ord'] > $ord_init and $dt_jrn_lieu['ord'] <= $val) {upd_noq("cat_jrn_lieu","ord","ord-1",$dt_jrn_lieu['id']);}
					if($dt_jrn_lieu['ord'] < $ord_init and $dt_jrn_lieu['ord'] >= $val) {upd_noq("cat_jrn_lieu","ord","ord+1",$dt_jrn_lieu['id']);}
				}
			}
		}
		elseif($tab=="cat_prs_lieu") {
			...
			if($val>$max[0]) {$flg_col = false;}
				...
				while($dt_prs_lieu = ftc_ass($rq_prs_lieu)) {
					if($dt_prs_lieu['ord'] > $ord_init and $dt_prs_lieu['ord'] <= $val) {upd_noq("cat_prs_lieu","ord","ord-1",$dt_prs_lieu['id']);}
					if($dt_prs_lieu['ord'] < $ord_init and $dt_prs_lieu['ord'] >= $val) {upd_noq("cat_prs_lieu","ord","ord+1",$dt_prs_lieu['id']);}
				}
			}
		}
...
elseif($col=="dt_min" || $col=="dt_max") {
	if($val!='') {
		...
		if(!isset($dt[2])) {
			$flg_y = true;
			if(strtotime(date("Y").'-'.$dt[1].'-'.$dt[0])>=strtotime(date("Y-m-d")) or $col=="dt_min") {$y=date("Y");}
			else{$y=date("Y")+1;}
		}
		else{$y=$dt[2];}
		$val = $y.'-'.$dt[1].'-'.$dt[0];
...
if($tab=='cat_mdl_jrn' and $col=='opt') {
	...
	$dt_jrn = ftc_ass(select('id','cat_mdl_jrn','id_mdl = '.$_POST["id_sup"].' and opt = 1 and ord',$dt_ord_jrn['ord']));
	upd_quo('cat_mdl_jrn','opt','0',$dt_jrn['id']);
}
elseif($tab=='cat_jrn_prs' and $col=='opt') {
	$dt_ord_prs = ftc_ass(select('ord','cat_jrn_prs','id',$id));
	$dt_prs = ftc_ass(select('id','cat_jrn_prs','id_jrn = '.$_POST["id_sup"].' and opt = 1 and ord',$dt_ord_prs['ord']));
	upd_quo('cat_jrn_prs','opt','0',$dt_prs['id']);
}
elseif($tab=='cat_prs_hbr' and $col=='opt') {
	$dt_hbr = ftc_ass(select('id','cat_prs_hbr','opt = 1 and id_prs',$_POST["id_sup"]));
		...
		while($dt_srv_trf = ftc_ass($rq_srv_trf)) {
			if($dt_srv_trf['id_frn']!=0) {
				...
				while($dt_frn_vll = ftc_ass($rq_frn_vll)) {
					if($dt_frn_vll['id_vll']==$val) {$flg_frn_vll = false;}
				}
				if($flg_frn_vll) {upd_quo("cat_srv_trf_bss","id_frn","NULL",$dt_srv_trf['id']);}
			}
		}
	}
	if($col=='ctg') {
		...
		while($dt_srv_trf = ftc_ass($rq_srv_trf)) {
			if($dt_srv_trf['id_frn']!=0) {
				...
				while($dt_frn_ctg_srv = ftc_ass($rq_frn_ctg_srv)) {
					if($dt_frn_ctg_srv['ctg_srv']==$val) {$flg_frn_ctg_srv = false;}
				}
				if($flg_frn_ctg_srv) {upd_quo("cat_srv_trf_bss","id_frn","NULL",$dt_srv_trf['id']);}
			}
		}
	}
}
elseif($tab=='cat_srv_trf' and $col=='def') {
	...
	while($dt_trf = ftc_ass($rq_trf)) {
		if($id != $dt_trf['id']) {upd_quo('cat_srv_trf','def','0',$dt_trf['id']);}
	}
}
elseif($tab=='cat_srv_trf_bss' and ($col=='bs_min' or $col=='bs_max')) {
	...
	while($dt_bss = ftc_ass($rq_bss)) {
		...
		if($col=='bs_max' and $dt_trf['bs_min']!='0' and $dt_trf['bs_min'] < $dt_bss['bs_max'] and $val > $dt_bss['bs_min'] and $val != 0) {
			echo $txt->errbss3->$id_lng;
			return;
		}
	//	if($col=='bs_min' and $dt_trf['bs_max']!='0' and $dt_trf['bs_max'] > $dt_bss['bs_min'] and $val < $dt_bss['bs_max']) {upd_quo('cat_srv_trf_bss','bs_max',$val,$id);}
	}
}
elseif($tab=='cat_srv_trf_ssn' and ($col=="dt_min" || $col=="dt_max")) {
	...
	while($dt_trf = ftc_ass($rq_trf)) {
		...
		if(($col=='dt_max' and $dt_srv['dt_min']!='0000-00-00' and strtotime($dt_srv['dt_min']) < strtotime($dt_trf['dt_max']) and strtotime($val) > strtotime($dt_trf['dt_min'])) or ($col=='dt_min' and $dt_srv['dt_max']!='0000-00-00' and strtotime($dt_srv['dt_max']) > strtotime($dt_trf['dt_min']) and strtotime($val) < strtotime($dt_trf['dt_max']))) {
			echo $txt->errdat3->$id_lng;
			return;
		}
	}
}
elseif($tab=='cat_hbr' and $col=='id_vll') {delete('cat_vll_hbr',"id_hbr",$id);}
elseif($tab=='cat_hbr_chm' and $col=='rgm') {delete('cat_vll_hbr',"id_chm",$_POST["id_sup"]);}
elseif($tab=='cat_hbr_chm_trf' and $col=='def') {
	...
	while($dt_trf = ftc_ass($rq_trf)) {
		if($id != $dt_trf['id']) {upd_quo('cat_hbr_chm_trf','def','0',$dt_trf['id']);}
	}
}
elseif($tab=='cat_hbr_chm_trf_ssn' and ($col=="dt_min" || $col=="dt_max")) {
	...
	while($dt_trf = ftc_ass($rq_trf)) {
		...
		if(($col=='dt_max' and $dt_chm['dt_min']!='0000-00-00' and strtotime($dt_chm['dt_min']) < strtotime($dt_trf['dt_max']) and strtotime($val) > strtotime($dt_trf['dt_min'])) or ($col=='dt_min' and $dt_chm['dt_max']!='0000-00-00' and strtotime($dt_chm['dt_max']) > strtotime($dt_trf['dt_min']) and strtotime($val) < strtotime($dt_trf['dt_max']))) {
			echo $txt->errdat3->$id_lng;
			return;
		}
	}
...
elseif($tab=='cat_hbr_rgm_trf' and $col=='def') {
	...
	while($dt_trf = ftc_ass($rq_trf)) {
		if($id != $dt_trf['id']) {upd_quo('cat_hbr_rgm_trf','def','0',$dt_trf['id']);}
	}
}
elseif($tab=='cat_hbr_rgm_trf_ssn' and ($col=="dt_min" || $col=="dt_max")) {
	...
	while($dt_trf = ftc_ass($rq_trf)) {
		...
		if(($col=='dt_max' and $dt_rgm['dt_min']!='0000-00-00' and strtotime($dt_rgm['dt_min']) < strtotime($dt_trf['dt_max']) and strtotime($val) > strtotime($dt_trf['dt_min'])) or ($col=='dt_min' and $dt_rgm['dt_max']!='0000-00-00' and strtotime($dt_rgm['dt_max']) > strtotime($dt_trf['dt_min']) and strtotime($val) < strtotime($dt_trf['dt_max']))) {
			echo $txt->errdat3->$id_lng;
			return;
		}
	}
}
elseif($col=='web' and $val != '' and (substr($val, 0, 7) != 'http://' and substr($val, 0, 8) != 'https://')) {$val = 'http://'.str_replace(" ","",trim($val)).'/';}


/*elseif($col=='titre' and $tab != 'cat_prs_txt' and $tab != 'cat_jrn_txt' and $tab != 'cat_frn' and $tab != 'cat_srv_txt' and $tab != 'cat_hbr_txt' and $tab != 'cat_hbr_chm_txt' and $tab != 'cat_lieu_txt') {$val = mb_strtoupper($val);}
utiliser upnoac() pour output word pdf (accent ok pour site web)?*/
if($col=='titre' or $col=='dsc') {
	$val = str_replace(array("[","{"),"(",$val);
	$val = str_replace(array("]","}"),")",$val);
	if($tab=='cat_crc_txt' or $tab=='cat_mdl_txt' or $tab=='cat_jrn_txt' or $tab=='cat_prs_txt' or $tab=='cat_vll_txt') {upd_quo($tab,array("dt_txt","usr"),array(date("Y-m-d H:i:s"),$id_usr),$id);}
}
if(($col=='duree' or $col=='chk_in' or $col=='chk_out') and $val=='') {$res = upd_nul($tab,$col,$id);}
else{$res = upd_quo($tab,$col,trim(addslashes($val)),$id);}
echo $res;
?>
