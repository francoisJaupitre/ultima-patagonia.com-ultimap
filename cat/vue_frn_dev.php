<?php
$rq_cnf = select("id_crc,groupe,version,dev_srv.opt,id_dev_prs","dev_srv LEFT JOIN (SELECT MIN(date),id_crc,groupe,version,cnf,dev_prs.id AS id_dev_prs FROM dev_srv INNER JOIN (dev_prs INNER JOIN (dev_jrn INNER JOIN (dev_mdl INNER JOIN dev_crc ON dev_mdl.id_crc = dev_crc.id) ON dev_jrn.id_mdl = dev_mdl.id) ON dev_prs.id_jrn = dev_jrn.id) ON dev_srv.id_prs = dev_prs.id WHERE dev_srv.id_frn = ".$id." GROUP BY id_crc) t ON dev_srv.id_prs = t.id_dev_prs","cnf","1","opt DESC,groupe,version","DISTINCT");
