...
foreach($res_srv[$id_lng] as $res_id => $nom){
	if($res_id != $dt_res['id_'.$cbl.'_res'] and substr(upnoac($nom),0,strlen($src)) == $src){
		...
		elseif($cbl == 'hbr'){$event = "maj('dev_hbr','res',".$res_id.",".$dt_res['id_dev_hbr'].",".$dt_res['id_prs'].",".$dt_res['id_crc'].");searchHbr(".$dt_res['id_cat_hbr'].",".$dt_res['id_cat_chm'].",0,".$dt_res['id_rgm'].",".$dt_res['id_dev_hbr'].",0,".$res_id.','.$dt_res['id_crc'].");";}
?>
