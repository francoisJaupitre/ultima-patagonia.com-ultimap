var cancel = flg_rch = false,timer,ld =/* ypos =*/ idpos = 0,flg_ld = {};
...
function richtext(elem,tab,col,id){
	if(flg_rch){
		var loader = 'ld_'+elem;
		var ed = elem;
		...
		tinymce.init({
			...
			paste_preprocess : function(pl, o) {o.content = strip_tags( o.content,'<b><strong><i><em><u>');},
			...
			menubar: false,
			save_onsavecallback: function(elem){
				maj(tab,col,elem.getContent(),id);
				elem.getBody().style.backgroundColor = "#FFFFFF";
			},
			setup : function(elem){
				elem.on('init', function(e){
					document.getElementById(loader).style.display='none';		// FIRST HIDE LOADER
					elem.execCommand('mceFocus',true,ed); 						// THEN FOCUS (ELSE USING TAB KEY, DUPLICATE FUNCTION THEN BUG!) // FJ 17-12-07

				});
				elem.on('change redo undo', function(e){elem.getBody().style.backgroundColor = "#FFFF66";});
			},
		});
...
function chk_rchtxt(){
	if(typeof tinyMCE !== "undefined" && tinyMCE.activeEditor){
		var rich = document.getElementsByClassName("rich");
	...
	return true;// ajouter textes a txt_js.xml
}
