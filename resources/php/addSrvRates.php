<?php //INSERT IN A QUOTATION SERVICES RATES FOR A NEW BASE PASSENGER FROM CATALOG
			...
			if($id_cat_srv != 0)
			{
				if($date != '0000-00-00')
				{
					$dt_trf1 = ftc_all(sel_quo(
						"*",
						"cat_srv_trf
							INNER JOIN cat_srv_trf_bss ON cat_srv_trf.id = cat_srv_trf_bss.id_trf
							INNER JOIN cat_srv_trf_ssn ON cat_srv_trf.id = cat_srv_trf_ssn.id_trf",
						"id_srv",
						$id_cat_srv
					));
				}
				...
				if($flg_trf)
				{
					...
					upd_quo(
						"dev_srv",
						array("crr", "taux", "sup", 'dt_min', 'dt_max'),
						array($cur, $taux, $sup, $dt_min, $dt_max),
						$id_dev_srv
					);
				}else{
