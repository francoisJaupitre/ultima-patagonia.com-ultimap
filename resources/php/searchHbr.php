<?php //GET ACCOMODATION SERVICES ACCORDING TO ACCOMODATION CRITERIA IN QUOTATIONS AND OPERATIONS
			...
			while($dt_prs = ftc_ass($rq_prs))
			{
				...
				while($dt_hbr = ftc_ass($rq_hbr))
				{
					if(
						$id_dev_prs != 0
						and $id_dev_hbr != 0
						and $dt_hbr['id_vll'] == $id_hbr_vll
						and $dt_hbr['rgm'] == $id_hbr_rgm
						and $dt_hbr['id_cat'] == '-1'
						and $dt_hbr['id'] != $id_dev_hbr
					)
					{ //NOT DEFINED ACCOMODATION
						$arr[] = $dt_hbr['id'];
					}elseif(
						$id_dev_prs != 0
						and $id_dev_hbr != 0
						and $dt_hbr['id_vll'] == $id_hbr_vll
						and $dt_hbr['rgm'] == $id_hbr_rgm
						and $dt_hbr['id_cat'] == $id_cat_hbr
						and $dt_hbr['id_cat_chm'] == '-1'
						and $dt_hbr['id'] != $id_dev_hbr
					)
					{ //DEFINED ACCOMODATION / NOT DEFINED ROOM
						$arr[] = $dt_hbr['id'];
					}elseif(
						$id_dev_prs != 0
					 	and $id_dev_hbr != 0
						and $id_hbr_vll == 0
						and $dt_hbr['id_cat'] == $id_cat_hbr
						and $dt_hbr['id_cat_chm'] == $id_cat_chm
						and $dt_hbr['id'] != $id_dev_hbr
						and $id_dev_prs != $dt_prs['id']
						and $dt_hbr[$res] == 0
					)
					{ //FOR REMOVE ACCOMODATION / FOR UPDATE SEL OR OPT VALUES
						$arr[] = array($dt_hbr['id'], $dt_prs['id']);
					}elseif($id_dev_prs != 0 and $id_dev_hbr == 0 and $dt_prs['id'] != $id_dev_prs)
					{ //ADD OPTIONAL ACCOMODATION
						if(
							(($cnf < 1 and $dt_hbr['opt'] == 1) or ($cnf > 0 and $dt_hbr['sel'] == 1))
							and $dt_hbr['id_cat'] == $id_ref_hbr
							and $dt_hbr['id_cat_chm'] == $id_ref_chm
							and $dt_hbr['rgm'] == $id_ref_rgm
							and $dt_hbr['id_vll'] == $id_ref_vll
						)
						{ //IF HAS SAME SELECTED ACCOMODATION
							$flg = true;
						}
						if(
							(($cnf < 1 and $dt_hbr['opt'] == 0) or ($cnf > 0 and $dt_hbr['sel'] == 0))
							and $dt_hbr['id_cat'] == $id_cat_hbr
							and $dt_hbr['id_cat_chm'] == $id_cat_chm
						)
						{ //ELSE IF THE OPTION ALREADY EXISTS
							$flg = false;
						}
					}elseif(
						$id_dev_prs == 0 and $id_dev_hbr != 0 and $dt_hbr['id_cat'] == $id_cat_hbr and $dt_hbr['id_cat_chm'] == $id_cat_chm
						and (
							($dt_hbr['res'] != $res and $dt_hbr['res'] != -1 and $dt_hbr['res'] != 6 and $dt_hbr['id'] != $id_dev_hbr)
							or ($res == 0 and $id_dev_hbr == 0)
						)
					)
					{ // FOR UPDATE RES VALUES
						$arr[] = $dt_hbr['id'];
					}
					elseif(
						$id_dev_prs == 0 and $id_dev_hbr == 0
						and $dt_hbr['id_cat'] == $id_cat_hbr
						and $dt_hbr['id_cat_chm'] == $id_cat_chm
					)
					{
						if($res == 'updateRates')
						{ //FOR UPDATE ALL NIGHT RATES
							$arr[] = $dt_hbr['id'];
						}elseif($res == 'sup')
						{ //FOR REMOVE ALL NIGHT
							$arr[] = array($dt_hbr['id'], $dt_prs['id']);
						}
					}
				}
