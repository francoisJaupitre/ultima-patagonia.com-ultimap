<?php //CREATE AN EMAIL PRINTING ACCOMODATIONS DATAS INCLUDED IN A QUOTATION FOR ONE OR ALL SUPPLIERS
$request = file_get_contents("php://input");
$data = json_decode($request, true);
if(isset($data['id_dev_crc']) and isset($data['id_res_hbr']) and isset($data['id_res_chm']) and isset($data['res']))
{
		...
		foreach($lst_dev as $id_dev)
		{
			if(isset($tab_hbr[$id_dev]))
			{
				foreach($tab_hbr[$id_dev] as $i => $hb)
				{
					...
					if(empty($mel_hbr))
					{
						$rsp .= $txt->mel_hbr->msg1->$id_lng.$nom_hbr.$txt->mel_hbr->msg2->$id_lng;
					}
					if($flg_send[$id_dev][$hb])
					{
						...
						if($ctg_res_hbr != 1)
						{
							$msg .= '<strong>'.$txt->mail->$id_lng.' '.$ctg_res[$id_lng][$ctg_res_hbr].'</strong>';
							if($ctg_res_hbr == 2)
							{
								$msg .= $tel_hbr;
							}
						...
						if($res == 0)
						{
							$subj = utf8_encode('Nueva cotizacion: '.$groupe.' / '.replace($nom_hbr));
						}elseif($cnf[$id_dev] > 0)
						{
							$subj = utf8_encode('Nueva reserva: '.$groupe.' / '.replace($nom_hbr));
						}elseif($cnf[$id_dev] < 1)
						{
							$subj = utf8_encode('Nuevo bloqueo: '.$groupe.' / '.replace($nom_hbr));
						}
						if(count($tab_hbr[$id_dev]) > 1 or empty($mel_usr))
						{
							$from = $mel_usr;
							$act = 'nv';
							$melto = $mel_hbr;
							...
							if($res > 0)
							{
								...
								{
									upd_quo("dev_hbr", array("res", "dt_res"), array($res, date("Y-m-d")), $id_srv);
									$rsp_hbr .= '|'.$id_srv;
								}
								$rq_res = sel_quo("id", "grp_res", array("id_grp", "id_hbr"), array($id_grp[$id_dev], $hb));
								if(num_rows($rq_res) == 0)
								{
									insert("grp_res", array("id_grp", "id_hbr"), array($id_grp[$id_dev], $hb));
								}
							}
						}else{
							$msg .= utf8_decode("
