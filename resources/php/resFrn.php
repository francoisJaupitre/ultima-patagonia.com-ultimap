<?php //GET SERVICE DATAS FOR PRINTING EMAILS AND DOCUMENTS IN QUOTATIONS AND OPERATIONS
...
$txt_res = simplexml_load_file('../xml/resTxt.xml');
if($id_dev_crc > 0)
{
	$rq_dev = sel_quo("dev_crc.*, grp_dev.id_clt", "dev_crc INNER JOIN grp_dev ON dev_crc.id_grp = grp_dev.id", "dev_crc.id", $id_dev_crc);
}else{
	$rq_dev = sel_quo("dev_crc.*, grp_dev.id_clt", "dev_crc INNER JOIN grp_dev ON dev_crc.id_grp = grp_dev.id", "dev_crc.cnf", "1");
}
while($dt_dev = ftc_ass($rq_dev))
{
	...
	if($obj != 'vch')
	{
		$lgg_crc[$id_dev] = $lgg_pys;
	}else{
		$lgg_crc[$id_dev] = $dt_dev['lgg'];
	}
	...
	while($dt_mdl = ftc_ass($rq_mdl))
	{
		...
		while($dt_jrn = ftc_ass($rq_jrn))
		{
			...
			while($dt_prs = ftc_ass($rq_prs))
			{
				...
				{
					if($guia_ctg_srv[$dt_srv['ctg']])
					{
						$prs_guia[$id_dev_prs] = 1;
					}
					if(
						(
					...
					)
					{
						...
						}elseif($id_cat_prs <= 0 and $id_res_frn == $id_frn)
						{
							...
							)
							{
								...
								if(!empty($old_dat[$fr]))
								{
									foreach($old_dat[$fr] as $o => $old)
									{
										$old_dat[$fr][$o] = $date_jrn;
									}
								}else{
						...
						if($dt_mdl['trf'])
						{
							$flg_send[$id_dev][$fr] = $flg_send_mdl;
						}else{
							$flg_send[$id_dev][$fr] = $flg_send_crc;
						}
	...
	if($obj != 'vch' and isset($tab_frn[$id_dev]))
	{
		foreach($tab_frn[$id_dev] as $i => $fr)
		{
			...
			{
				foreach(array_unique($tab_nb_pax_fr[$id_dev][$fr]) as $pax)
				{
					$nbpx .= $pax.' / ';
				}
			...
			foreach($tab_prs[$id_dev][$fr] as $j => $pr)
			{
				...
				{
					if($tab_in[$id_dev][$fr][$j] != '0000-00-00')
					{
						$dat = date("d/m/Y", strtotime($tab_in[$id_dev][$fr][$j]));
					}else{
						$dat = '';
					}
					if($tab_out[$id_dev][$fr][$j] != $tab_in[$id_dev][$fr][$j] and !empty($tab_out[$id_dev][$fr][$j]) and $tab_ctg_prs[$id_dev][$fr][$j] != 19 and $tab_ctg_prs[$id_dev][$fr][$j] != 20)
					{
						$dat .= ' - '.date("d/m/Y", strtotime($tab_out[$id_dev][$fr][$j]));
					}
					if(isset($old_dat) and ($dat != $old_dat or $pr != $old_prs))
					{
						$message[$id_dev][$fr] .= '<br />';
					}
					if(!isset($old_dat) or $dat != $old_dat)
					{
						$message[$id_dev][$fr] .= '<br />'.$dat.':<br />';
						$old_dat = $dat;
					}
				}
				if(!isset($old_prs_id[$fr]) or $tab_prs_id[$id_dev][$fr][$j] != $old_prs_id[$fr])
				{
					...
					if($flg_mlt_pax)
					{
						$message[$id_dev][$fr] .= ' x'.$tab_nb_pax_fr[$id_dev][$fr][$j].'<br />';
					}
					$old_prs = $pr;
					...
					if(!isset($lieu_prs[$id_dev][$pr]))
					{
						...
						{
							$lieu_prs[$id_dev][$pr][] = $dt_prs_lieu['id_lieu'];
						}
					}
					$flg_rtc = true;
					if($tab_ctg_prs[$id_dev][$fr][$j] == 10)
					...
					if($flg_rtc)
					{
						if($tab_ctg_prs[$id_dev][$fr][$j] != 10)
						{
							...
							{
								$message[$id_dev][$fr] .= date("H:i", strtotime($tab_hre[$id_dev][$fr][$j])).$txt->res_frn->hs->$id_lgg.": ";
							}
							if(isset($lieu_prs[$id_dev][$pr])){
								$message[$id_dev][$fr] .= $ctg_prs[$id_lgg_frn][$tab_ctg_prs[$id_dev][$fr][$j]]." ";
							}
							if(!$dt_cat_prs['is_out']){
								$message[$id_dev][$fr] .= $tab_info[$id_dev][$fr][$j]." ";
							}
						}
						if(!empty($dt_cat_prs['ref']))
						{
							$message[$id_dev][$fr] .= '['.$dt_cat_prs['ref'].'] ';
						}
						...
						if(!isset($ord[$fr][$id_jrn]))
						{
							$ord[$fr][$id_jrn]=1;
						}
						if(!isset($new_jrn[$fr]) or $id_jrn != $new_jrn[$fr])
						{
							$new_jrn[$fr] = $id_jrn;
							...
							while($dt_dev_prs = ftc_ass($rq_dev_prs))
							{
								...
								while($dt_dev_hbr = ftc_ass($rq_dev_hbr))
								{
									...
									)
									{
										...
										if($dt_cat_hbr['titre'] == '')
										{
											$new_hbr_nom[$fr] = $dt_cat_hbr['nom'];
										}else{
											$new_hbr_nom[$fr] = $dt_cat_hbr['titre'];
										}
							...
							if($flg_new and !$dt_cat_prs['is_out'])
							{
								if(!$dt_prs['fus'])
								{
									if(isset($old_hbr_nom[$fr]))
									{
										$new_hbr_nom[$fr] = $old_hbr_nom[$fr];
									}else{
										$new_hbr_nom[$fr] = 'NO HOTEL!';
									}
								}else{
						...
						if($flg_old or !isset($old_hbr_nom[$fr]))
						{
							...
							if(!isset($old_jrn[$pr]) or $dt_jrn['id'] != $old_jrn[$pr])
							{
								$old_jrn[$pr] = $dt_jrn['id'];
								...
								while($dt_dev_prs = ftc_ass($rq_dev_prs))
								{
									...
									while($dt_dev_hbr = ftc_ass($rq_dev_hbr))
									{
										if($cnf[$id_dev] == 1 and ($dt_dev_hbr['sel'] == 1 or ($dt_dev_hbr['opt'] == 1 and $dt_dev_prs['res'] == -1)) and $dt_dev_hbr['id_cat'] != 0)
										{
											...
											if($dt_cat_hbr['titre'] == '')
											{
												$old_hbr_nom[$fr] = $dt_cat_hbr['nom'];
											}else{
												$old_hbr_nom[$fr] = $dt_cat_hbr['titre'];
											}
						...
						if(isset($lieu_prs[$id_dev][$pr]))
						{
							foreach($lieu_prs[$id_dev][$pr] as $id_lieu)
							{
								...
								if($dt_lieu['hbr'])
								{
									if($ord[$fr][$id_jrn] == 1 and isset($old_hbr_nom) and $old_hbr_nom[$fr] != '' and $old_vll[$fr] == $dt_lieu['id_vll'])
									{
										$message[$id_dev][$fr] .= $old_hbr_nom[$fr].' ('.$vll[$old_vll[$fr]].')';
									}elseif($new_hbr_nom[$fr] != '' and $new_vll[$fr] == $dt_lieu['id_vll'])
									{
										$message[$id_dev][$fr] .= $new_hbr_nom[$fr].' ('.$vll[$new_vll[$fr]].')';
									}elseif(isset($old_hbr_nom) and $old_hbr_nom[$fr] != '' and $old_vll[$fr] == $dt_lieu['id_vll'])
									{
										$message[$id_dev][$fr] .= $old_hbr_nom[$fr].' ('.$vll[$old_vll[$fr]].')';
									}else{
										$flg_hbr = false;
								...
								if(!$flg_hbr)
								{
									if($dt_lieu['titre'] == '')
									{
										$message[$id_dev][$fr] .= $dt_lieu['nom'];
									}else{
										$message[$id_dev][$fr] .= $dt_lieu['titre'];
									}
								}
								$ord[$fr][$id_jrn]++;
							}
						}elseif($dt_cat_prs['titre'] == '')
						{
							$message[$id_dev][$fr] .= $dt_cat_prs['nom'];
						}else{
							$message[$id_dev][$fr] .= $dt_cat_prs['titre'];
						}
						if($dt_cat_prs['is_out'])
						{
							$message[$id_dev][$fr] .= " OUT: ".$tab_info[$id_dev][$fr][$j];
						}
						if(!is_null($dt_cat_prs['duree']))
						{
							$message[$id_dev][$fr] .= '<br />- '.$txt->res_frn->duree->$id_lgg_frn.': ';
							if(date("i", strtotime($dt_cat_prs['duree'])) == '00')
							{
								$message[$id_dev][$fr] .= date("H", strtotime($dt_cat_prs['duree'])).$txt->res_frn->hs->$id_lgg." ";
							}else{
								$message[$id_dev][$fr] .= date("H:i", strtotime($dt_cat_prs['duree'])).$txt->res_frn->hs->$id_lgg." ";
							}
						}
						...
						while($dt_srv = ftc_ass($rq_srv))
						{
							if(!isset($old_frn) or $dt_srv['id_frn'] != $old_frn or $dt_srv['ctg'] != $old_ctg or ($dt_srv['nom'] != $old_nom and ($mrk_nom_ctg_srv[$dt_srv['ctg']] or $mrk_ctg_ctg_srv[$dt_srv['ctg']])))
							{
								...
								if($dt_srv['id_frn'] != $fr)
								{
									if($mrk_ctg_ctg_srv[$dt_srv['ctg']] or $mrk_nom_ctg_srv[$dt_srv['ctg']])
									{
										...
										if($mrk_ctg_ctg_srv[$dt_srv['ctg']])
										{
											...
											if($lgg_ctg_srv[$dt_srv['ctg']] and $dt_srv['lgg']>0)
											{
												$message[$id_dev][$fr] .= '('.$nom_lgg_lgg[$id_lgg_frn][$dt_srv['lgg']].') ';
											}
										}
										if($mrk_nom_ctg_srv[$dt_srv['ctg']])
										{
											if($dt_srv['titre'] == '')
											{
												...
												if(!$mrk_ctg_ctg_srv[$dt_srv['ctg']] and $lgg_ctg_srv[$dt_srv['ctg']] and $dt_srv['lgg']>0)
												{
													$message[$id_dev][$fr] .= '('.$nom_lgg_lgg[$id_lgg_frn][$dt_srv['lgg']].') ';
												}
											}else{
												$message[$id_dev][$fr] .= ' '.$dt_srv['titre'];
											}
										}
										if($dt_srv['id_frn']>0)
										{
											...
											$message[$id_dev][$fr] .= ": SIN DEFINIR";
										}
									}
								}elseif(
									$mrk_srv_ctg_prs[$tab_ctg_prs[$id_dev][$fr][$j]] and (
										$mrk_ctg_ctg_srv[$dt_srv['ctg']] or $mrk_nom_ctg_srv[$dt_srv['ctg']]
									)
								)
								{
									$message[$id_dev][$fr] .= '<br />-';
									if($srv_ctg_srv[$dt_srv['ctg']])
									{
										$message[$id_dev][$fr] .= ' '.$txt->res_frn->msg6->$id_lgg_frn;
									}
									if($mrk_ctg_ctg_srv[$dt_srv['ctg']])
									{
										$message[$id_dev][$fr] .= ' '.$ctg_srv[$id_lgg_frn][$dt_srv['ctg']].' ';
									...
									if($dt_srv['res'] > 1)
									{
										$message[$id_dev][$fr] .= ": ".$res_srv[$id_lgg_frn][$dt_srv['res']];
									}else{
										$message[$id_dev][$fr] .= ": ".$txt->res_frn->compris->$id_lgg.' ';
									}
								}
							}
						}
					...
					if($dt_mdl['trf'])
					{
						$rq_rmn_pax = sel_quo("*", "dev_mdl_pax INNER JOIN grp_pax ON dev_mdl_pax.id_pax = grp_pax.id", "id_mdl", $tab_mdl[$id_dev][$fr][$j], "ord");
					}else{
						$rq_rmn_pax = sel_quo("*", "dev_crc_pax INNER JOIN grp_pax ON dev_crc_pax.id_pax = grp_pax.id", "id_crc", $id_dev, "ord");
					}
					if(num_rows($rq_rmn_pax) > 0)
					{
						while($dt_rmn_pax = ftc_ass($rq_rmn_pax))
						{
							...
							if($dt_rmn_pax['dob'] != '0000-00-00')
							{
								$mespxlst .= date("d/m/Y", strtotime($dt_rmn_pax['dob']));
							}
							$mespxlst .= '</td><td>'.stripslashes($dt_rmn_pax['psp']).'</td><td>';
							if($dt_rmn_pax['exp'] != '0000-00-00')
							{
								$mespxlst .= date("d/m/Y", strtotime($dt_rmn_pax['exp']));
							}
