<?php //GET ACCOMODATION DATAS FOR PRINTING EMAILS AND DOCUMENTS IN QUOTATIONS AND OPERATIONS
...
while($dt_dev = ftc_ass($rq_dev))
{
	...
	while($dt_rmn = ftc_ass($rq_rmn))
	{
		...
		while($dt_pax = ftc_ass($rq_pax))
		{
			...
			if($rm > 0)
			{
				if(isset($pax[0][$id_rmn]))
				{
					$pax[0][$id_rmn] .= $room[$id_lgg_hbr][$rm];
				}else{
					$pax[0][$id_rmn] = $room[$id_lgg_hbr][$rm];
				}
				if($cpc_room[$rm] > 0)
				{
					$pax[0][$id_rmn] .= ': '.ceil($nc / $cpc_room[$rm]);
				}
				$pax[0][$id_rmn] .= " / ";
			}
		}
	...
	while($dt_mdl = ftc_ass($rq_mdl))
	{
		...
		if($dt_mdl['trf'])
		{
			...
			while($dt_rmn = ftc_ass($rq_rmn))
			{
				...
				while($dt_pax = ftc_ass($rq_pax))
				{
					...
					if($rm > 0)
					{
						...
						if($cpc_room[$rm] > 0)
						{
							$pax[1][$id_rmn] .= ': '.ceil($nc/$cpc_room[$rm]);
						}
						$pax[1][$id_rmn] .= " / ";
					}
				}
				...
				if(isset($pax[1]))
				{
					$pax[1][$id_rmn] = substr($pax[1][$id_rmn], 0, -3);
				}
			}
			$rom = '';
			if($dt_mdl['sgl'] > 0)
			{
				$rom = $room[$id_lgg_hbr][1].': '.$dt_mdl['sgl']." / ";
			}
			if($dt_mdl['dbl_mat'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][2].': '.$dt_mdl['dbl_mat']." / ";
			}
			if($dt_mdl['dbl_twn'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][3].': '.$dt_mdl['dbl_twn']." / ";
			}
			if($dt_mdl['tpl_mat'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][4].': '.$dt_mdl['tpl_mat']." / ";
			}
			if($dt_mdl['tpl_twn'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][5].': '.$dt_mdl['tpl_twn']." / ";
			}
			if($dt_mdl['qdp'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][6].': '.$dt_mdl['qdp']." / ";
			}
			if($dt_mdl['psg'])
			{
				$rom .= "+ SGL TL / ";
			...
			{
				$rom = $room[$id_lgg_hbr][1].': '.$dt_dev['sgl']." / ";
			}
			if($dt_dev['dbl_mat'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][2].': '.$dt_dev['dbl_mat']." / ";
			}
			if($dt_dev['dbl_twn'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][3].': '.$dt_dev['dbl_twn']." / ";
			}
			if($dt_dev['tpl_mat'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][4].': '.$dt_dev['tpl_mat']." / ";
			}
			if($dt_dev['tpl_twn'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][5].': '.$dt_dev['tpl_twn']." / ";
			}
			if($dt_dev['qdp'] > 0)
			{
				$rom .= $room[$id_lgg_hbr][6].': '.$dt_dev['qdp']." / ";
...
// preveer lista de fechas enviadas, confirmadas, anuladas etc.
						...
						if($id_cat_chm > -2)
						{
							...
							}elseif($id_cat_chm <= 0 and $id_res_hbr == $id_cat_hbr)
							{
								...
								if(!isset($hbr) or !in_array($id_cat_hbr, $hbr))
								{
									$flg_new_hbr = true;
								}
								$hb = $id_cat_hbr;
								...
								if(
									$flg_new_hbr
									or isset($flg_new_rgm) and $flg_new_rgm
									or isset($chm) and $chm[$hb][$rg][$ii[$hb][$rg]-1] != $id_cat_chm
								...
								)
								{
									if($dt_prs['ctg'] != 11 and $dt_prs['ctg'] != 17)
									{
										...
										if((($dt_mdl['trf'] and $p[$id_dev_mdl] > 1) or (!$dt_mdl['trf'] and $p[0] > 1)) and $pax[$dt_mdl['trf']][$dt_prs['id_rmn']])
										{
											$rmn[$hb][$rg][$ii[$hb][$rg]] = $pax[$dt_mdl['trf']][$dt_prs['id_rmn']];
										}else{
											$rmn[$hb][$rg][$ii[$hb][$rg]] = $rom;
										}
									...
									}elseif($dt_prs['ctg'] == 11)
									{
										...
										if(!is_null($dt_prs['heure']))
										{
											$hre_early[$hb][] = $dt_prs['heure'];
										}
										if((($dt_mdl['trf'] and $p[$id_dev_mdl] > 1) or (!$dt_mdl['trf'] and $p[0] > 1)) and $pax[$dt_mdl['trf']][$dt_prs['id_rmn']])
										{
											$rmn_early[$hb][$rg][$ii[$hb][$rg]] = $pax[$dt_mdl['trf']][$dt_prs['id_rmn']];
										}else{
											$rmn_early[$hb][$rg][$ii[$hb][$rg]] = $rom;
										}
									}elseif($dt_prs['ctg'] == 17)
									{
										...
										if(!is_null($dt_prs['heure']))
										{
											$hre_late[$hb][] = $dt_prs['heure'];
										}
										if((($dt_mdl['trf'] and $p[$id_dev_mdl] > 1) or (!$dt_mdl['trf'] and $p[0] > 1)) and $pax[$dt_mdl['trf']][$dt_prs['id_rmn']])
										{
											$rmn_late[$hb][$rg][$ii[$hb][$rg]] = $pax[$dt_mdl['trf']][$dt_prs['id_rmn']];
										}else{
											$rmn_late[$hb][$rg][$ii[$hb][$rg]] = $rom;
										}
									}
									if($dt_mdl['trf'])
									{
										$flg_send[$id_dev][$hb] = $flg_send_mdl;
									}else{
										$flg_send[$id_dev][$hb] = $flg_send_crc;
									}
									...
									{
										foreach($old_dat[$hb][$rg] as $o => $old) {
											$old_dat[$hb][$rg][$o] = $date_jrn;
										}
									}else{
							...
							$lst_hbr[$id_dev][$id_cat_hbr][] = $id_dev_hbr;
						}
					}
				}
			}
	...
	if(isset($hbr))
	{
		...
		if(isset($rmn_early))
		{
			$tab_rmn_early[$id_dev] = $rmn_early;
		}
		if(isset($rmn_late))
		{
			$tab_rmn_late[$id_dev] = $rmn_late;
		}
		...
		if(isset($hbr_early))
		{
			$tab_early[$id_dev] = $hbr_early;
		}
		if(isset($hre_early))
		{
			$tab_hre_early[$id_dev] = $hre_early;
		}
		if(isset($hbr_late))
		{
			$tab_late[$id_dev] = $hbr_late;
		}
		if(isset($hre_late))
		{
			$tab_hre_late[$id_dev] = $hre_late;
		}
		...
		foreach($tab_hbr[$id_dev] as $i => $hb)
		{
			...
			foreach($tab_rgm[$id_dev][$hb] as $r => $rg)
			{
				...
				foreach($tab_chm[$id_dev][$hb][$rg] as $j => $ch)
				{
					...
					{
						if(isset($message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]]))
						{
							$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= '<br />Categoria: '.$dt_cat_chm['nom'].'<br />';
						}else{
							$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] = '<br />Categoria: '.$dt_cat_chm['nom'].'<br />';
						}
					}
					...
					elseif(empty($rmg_hbr[$dt_cat_hbr['ctg']]))
					{
						$rsp .= $txt->res_hbr->msg8->$id_lng.".\n";
					}
					...
					if(isset($tab_early[$id_dev][$hb]))
					{
						if(in_array($tab_in[$id_dev][$hb][$rg][$j], $tab_early[$id_dev][$hb]))
						{
							...
							if(!is_null($tab_hre_early[$id_dev][$hb]))
							{
								$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= ' '.date("H:i", strtotime($tab_hre_early[$id_dev][$hb][$j])).$txt->res_frn->hs->$id_lgg;
							}
							if($tab_rmn[$id_dev][$hb][$rg][$j] != $tab_rmn_early[$id_dev][$hb][$rg][$j])
							{
								if(empty($tab_rmn_early[$id_dev][$hb][$rg][$j]))
								{
									$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= ' ('.$txt_res->no_rmn->$id_lgg_hbr.')';
								}else{
									$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= ' ('.$tab_rmn_early[$id_dev][$hb][$rg][$j].')';
								}
							}
						}
					}
					...
					if(isset($tab_late[$id_dev][$hb]))
					{
						if(in_array($tab_out[$id_dev][$hb][$rg][$j], $tab_late[$id_dev][$hb]))
						{
							...
							if(!is_null($tab_hre_late[$id_dev][$hb]))
							{
								$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= ' '.date("H:i", strtotime($tab_hre_late[$id_dev][$hb][$j])).$txt->res_frn->hs->$id_lgg;
							}
							if($tab_rmn[$id_dev][$hb][$rg][$j] != $tab_rmn_late[$id_dev][$hb][$rg][$j])
							{
								if(empty($tab_rmn_late[$id_dev][$hb][$rg][$j]))
								{
									$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= ' ('.$txt_res->no_rmn->$id_lgg_hbr.')';
								}else{
									$message[$id_dev][$hb][$rg][$ch][$tab_rmn_pax[$id_dev][$hb][$rg][$j]] .= ' ('.$tab_rmn_late[$id_dev][$hb][$rg][$j].')';
								}
							}
						}
					}
						...
						if($id_rmn > 0)
						{
							...
							while($dt_rmn_pax = ftc_ass($rq_rmn_pax))
							{
								if($flagTable)
								{
									...
									{
										$mesrmlst .= '<td>'.$txt_res->rmn->$id_lgg_hbr.':</td>';
									}
								...
								if($dt_rmn_pax['room'] > 0)
								{
									...
									if($dt_pax['dob'] != '0000-00-00')
									{
										$mesrmlst .= date("d/m/Y", strtotime($dt_pax['dob']));
									}
									$mesrmlst .= '</td><td>'.stripslashes($dt_pax['psp']).'</td><td>';
									if($dt_pax['exp'] != '0000-00-00')
									{
										$mesrmlst .= date("d/m/Y", strtotime($dt_pax['exp']));
									}
									$mesrmlst .= '</td><td>'.$ncn[$id_lgg_hbr][$dt_pax['ncn']].'</td>';
									if($rmg_hbr[$dt_cat_hbr['ctg']])
									{
										$mesrmlst .= '<td>'.$room[$id_lgg_hbr][$dt_rmn_pax['room']].': '.$dt_rmn_pax['nc']. '</td>';
									}
