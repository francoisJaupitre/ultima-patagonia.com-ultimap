	...
	switch($cbl)
	{
		case 'grp':
			...
			if(num_rows($num_crc)==0 and num_rows($num_fin)==0)
			{
				...
				echo json_encode($txt->del->grp->$id_lng);
			}
			break;
		case 'fin':
		case 'arc':
		...
		case 'mdl':
			...
			if(num_rows($rq_mdl_txt) == 0)
			{
				...
				echo json_encode($txt->del->mdl2->$id_lng);
			}
			break;
		case 'jrn':
		...
		case 'hbr':
			...
			if(empty($dt_vll_hbr['id']))
			{
				...
				while($dt_vll_hbr = ftc_ass($rq_vll_hbr))
				{
					$err = $dt_vll_hbr['nom']."\n";
				}
		...
		case 'pic':
			...
			if(empty($dt_jrn_pic['id']))
			{
				...
				if(unlink('../pic/'.$dir.'/'.$dt_pic['pic']))
				{
					delete("cat_pic","id",$id);
				}
			}else{
				echo json_encode($txt->del->pic->$id_lng);
			}
			break;
		case 'rgn':
			...
			if(empty($dt_vll['id']))
			{
				delete("cat_rgn","id",$id);
			}else{
				echo json_encode($txt->del->rgn->$id_lng);
			}
			break;
		case 'vll':
					...
					if(empty($dt_cat_hbr['id']))
					{
						...
						if(empty($dt_cat_frn['id']))
						{
							...
							if(empty($dt_cat_lieu['id']))
							{
								...
								echo json_encode($txt->del->vll_lieu->$id_lng);
							}
						}else{
							echo json_encode($txt->del->vll_frn->$id_lng);
						}
					}else{
						echo json_encode($txt->del->vll_hbr->$id_lng);
					}
				}else{
					echo json_encode($txt->del->vll_srv->$id_lng);
				}
			}else{
				echo json_encode($txt->del->vll_jrn->$id_lng);
			}
			break;
		case 'lieu':
			...
			if(empty($dt_prs_lieu['id']))
			{
				...
				echo json_encode($txt->del->lieu->$id_lng);
			}
			break;
		case 'bnq':
			...
			if(empty($dt_frn['id']))
			{
				...
				if(empty($dt_hbr['id']))
				{
					delete("cat_bnq","id",$id);
				}else{
					echo json_encode($txt->del->bnq_hbr->$id_lng);
				}
			}else{
				echo json_encode($txt->del->bnq_frn->$id_lng);
			}
			break;
	}
}
