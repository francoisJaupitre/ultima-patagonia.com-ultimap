<?php //CREATE A DOCX FILE PRINTING CONFIRMED ACCOMODATION VOUCHERS FOR ONE OR ALL SUPPLIERS
if(isset($_GET['id']) and $_GET['id'] > 0 and isset($_GET['hbr']) and $_GET['hbr'] >= 0)
{
	$id_dev_crc = $_GET['id'];
	$id_res_hbr = $_GET['hbr'];
	$id_res_chm = 0;
		...
		foreach($tab_hbr[$id_dev] as $i => $hb)
		{
			...
			foreach($tab_rgm[$id_dev][$hb] as $r => $rg)
			{
				foreach($tab_chm[$id_dev][$hb][$rg] as $j => $ch)
				{
					...
					$section->addText('', $fontStyle3, $paragraphStyle2);
					$section->addText('', $fontStyle3, $paragraphStyle2);
					if(!empty($dt_cat_hbr['titre']))
					{
						$section->addText(stripslashes(replace($dt_cat_hbr['titre'])), $fontStyle3, $paragraphStyle2);
					}else{
						$section->addText(stripslashes(replace($dt_cat_hbr['nom'])), $fontStyle3 , $paragraphStyle2);
					}
					$section->addText('', $fontStyle, $paragraphStyle2);
					if(!empty($dt_cat_hbr['adresse']))
					{
						$section->addText(stripslashes(replace($dt_cat_hbr['adresse'])), $fontStyle, $paragraphStyle2);
					}
					$section->addText(stripslashes(replace($vll[$dt_cat_hbr['id_vll']])), $fontStyle, $paragraphStyle2);
					if(!empty($dt_cat_hbr['tel_hbr']))
					{
						$section->addText(stripslashes(replace($dt_cat_hbr['tel_hbr'])), $fontStyle, $paragraphStyle2);
					}
					if(!is_null($dt_cat_hbr['chk_in']) and !is_null($dt_cat_hbr['chk_out']))
					{
						$section->addText('', $fontStyle, $paragraphStyle2);
						$section->addText(stripslashes(replace(
							$txt_vch->checkin->$id_lgg.': '.date("H:i", strtotime($dt_cat_hbr['chk_in'])).$txt_vch->hs->$id_lgg.' / '.$txt_vch->checkout->$id_lgg.': '.date("H:i", strtotime($dt_cat_hbr['chk_out'])).$txt_vch->hs->$id_lgg
						)), $fontStyle, $paragraphStyle2);
					}
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$table = $section->addTable($tableStyle);
					...
					if($tab_rva[$id_dev][$hb][$rg][$j] != '')
					{
						...
						if($id_lgg_hbr != $id_lgg)
						{
							$cell->addText('['.stripslashes(replace($txt_vch->rva->$id_lgg_hbr.']')), $fontStyle, $paragraphStyle2);
						}
					}
					$cell = $table->addCell(2000, $cellStyle);
					...
					if($id_lgg_hbr != $id_lgg)
					{
						$cell->addText('['.stripslashes(replace($txt_vch->sir->$id_lgg_hbr.']')), $fontStyle, $paragraphStyle2);
					}
					...
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$dt_cat_chm = ftc_ass(sel_quo(
						"cat_hbr_chm.*, cat_hbr_chm_txt.titre",
						"cat_hbr_chm LEFT JOIN cat_hbr_chm_txt ON cat_hbr_chm_txt.id_hbr_chm = cat_hbr_chm.id AND lgg = ".$lgg_crc[$id_dev],
						"cat_hbr_chm.id",
						$tab_chm[$id_dev][$hb][$rg][$j]
					));
					$table = $section->addTable($tableStyle);
					$table->addRow($height);
					...
					if($id_lgg_hbr != $id_lgg)
					{
						$cell->addText('['.stripslashes(replace($txt_vch->ctg->$id_lgg_hbr.']')), $fontStyle, $paragraphStyle2);
					}
					$cell = $table->addCell(7000, $cellStyle2);
					if(!empty($dt_cat_chm['titre']))
					{
						$cell->addText(stripslashes(replace($dt_cat_chm['titre'])), $fontStyle, $paragraphStyle2);
					}else{
						...
						if($lgg_hbr != $lgg_crc[$id_dev])
						{
							...
							if(!empty($dt_cat_chm_trad['titre']))
							{
								$cell->addText('['.stripslashes(replace($dt_cat_chm_trad['titre'])).']', $fontStyle, $paragraphStyle2);
							}
						}
					}
					$section->addText('', $fontStyle, $paragraphStyle2);
					...
					if($id_lgg_hbr != $id_lgg)
					{
						$cell->addText('['.stripslashes(replace($txt_vch->rgm->$id_lgg_hbr)).']', $fontStyle, $paragraphStyle2);
					}
					$cell = $table->addCell(3000, $cellStyle2);
					$cell->addText(stripslashes(replace($rgm[$id_lgg][$rg])), $fontStyle, $paragraphStyle2);
					if($id_lgg_hbr != $id_lgg)
					{
						$cell->addText('['.stripslashes(replace($rgm[$id_lgg_hbr][$rg])).']', $fontStyle, $paragraphStyle2);
					}
					$section->addText('', $fontStyle, $paragraphStyle2);
					...
					if(isset($tab_early[$id_dev][$hb]) and in_array($tab_in[$id_dev][$hb][$rg][$j], $tab_early[$id_dev][$hb]))
					{
						$msg .= ' ('.$txt_vch->early->$id_lgg.')';
					}
					...
					if(isset($tab_late[$id_dev][$hb]) and in_array($tab_out[$id_dev][$hb][$rg][$j], $tab_late[$id_dev][$hb]))
					{
							$msg .= ' ('.$txt_vch->late->$id_lgg.')';
					}
					$cell->addText(stripslashes(replace($msg)), $fontStyle, $paragraphStyle2);
					if($id_rmn > 0)
					{
