<?php //CREATE A NEW QUOTATION FROM CATALOG CIRCUITS OR WEBSITE FORM CLIENT EMAILS
...
$alt = array();
$err = $err_crc = $err_mdl = $err_jrn = $err_prs = $err_hbr = $err_srv = "";
  ...
  foreach($url as $u)
	{
		...
		}elseif($key == 'uid_mdl')
		{
			...
			$uid = $value;
			$data_web[$obj][$uid]['uid'] = $uid;
		}elseif($key == 'id_opt_jrn')
		{
			$data_web[$obj][$uid]['id_opt_jrn'][] = $value;
		}elseif($key == 'id_sel_jrn')
		{
			$id_sel_jrn = $value;
		}elseif($key == 'id_opt_prs')
		{
			$data_web[$obj][$uid]['id_opt_prs'][$id_sel_jrn][] = $value;
		}else{
			$data_web[$key] = $value;
		}
	}
...
if(isset($ids_dev_crc))
{
	$id_dev_crc = implode('|', $ids_dev_crc);
}
$qa = array($id_dev_crc, $err, implode(",\n", $alt));
echo json_encode($qa);
?>
