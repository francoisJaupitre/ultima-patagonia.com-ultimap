...
if(isset($data['obj']) and isset($data['lgg']) and $data['lgg'] > 0 and isset($data['id']) and $data['id'] > 0)
{
	...
	switch ($obj) {
		case 'crc':
			...
			if($id_cat_crc > 0)
			{
				$dt_jrn = ftc_ass(sel_quo("MIN(date) as date", "dev_jrn RIGHT JOIN (SELECT id,MIN(ord) AS ord FROM dev_mdl WHERE id_crc = ".$id_dev_crc." GROUP BY id) t ON dev_jrn.id_mdl = t.id"));
		...
		case 'mdl':
			$id_dev_mdl = $data['id'];
			$dt_dev_mdl = ftc_ass(sel_quo("dev_mdl.id_cat, crr, id_crc, ord, fus", "dev_mdl INNER JOIN dev_crc ON dev_mdl.id_crc = dev_crc.id", "dev_mdl.id", $id_dev_mdl));
