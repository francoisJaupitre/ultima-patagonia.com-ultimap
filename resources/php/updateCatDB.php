<?php //SET VALUE IN DATABASE FROM CATALOG
...
if(isset($data["tab"]) and isset($data["col"]) and isset($data["val"]) and isset($data["id"]))
{
	...
	}elseif($tab == 'cat_srv_trf_bss' and ($col == 'bs_min' or $col == 'bs_max'))
	{
		...
		if(
			($col == 'bs_max' and $dt_trf['bs_min'] > $val and $val != '0')
			or ($col == 'bs_min' and $dt_trf['bs_max'] < $val and $dt_trf['bs_max'] != '0' and $val !='0')
		)
		{
			echo json_encode($txt->errbss->$id_lng);
			return;
		}
		...
		while($dt_bss = ftc_ass($rq_bss))
		{
			...
			{
				echo json_encode($txt->errbss2->$id_lng);
				return;
			}elseif($col == 'bs_max' and $dt_trf['bs_min']!='0' and $dt_trf['bs_min'] < $dt_bss['bs_max'] and $val > $dt_bss['bs_min'] and $val != 0)
			{
				echo json_encode($txt->errbss3->$id_lng);
				return;
			}
		}
	}elseif($tab == 'cat_srv_trf_ssn' and ($col == "dt_min" || $col == "dt_max"))
	{
		$dt_srv = ftc_ass(sel_quo(
			'id_srv, dt_min, dt_max',
			'cat_srv_trf INNER JOIN cat_srv_trf_ssn ON cat_srv_trf.id = cat_srv_trf_ssn.id_trf',
			'cat_srv_trf_ssn.id',
			$id)
		);
		if(
			($col == 'dt_max' and strtotime($dt_srv['dt_min']) > strtotime($val) and $val !='0000-00-00' and !$flg_y)
			or ($col == 'dt_min' and strtotime($dt_srv['dt_max']) < strtotime($val) and $dt_srv['dt_max'] != '0000-00-00')
		)
		{
			echo json_encode($txt->errdat->$id_lng);
			return;
		...
		$rq_trf = sel_whe(
			'cat_srv_trf_ssn.id, cat_srv_trf_ssn.dt_min, cat_srv_trf_ssn.dt_max',
			'cat_srv_trf INNER JOIN cat_srv_trf_ssn ON cat_srv_trf.id = cat_srv_trf_ssn.id_trf',
			'cat_srv_trf_ssn.id!='.$id.' AND id_srv = '.$dt_srv['id_srv']
		);
		while($dt_trf = ftc_ass($rq_trf))
		{
			if(strtotime($val) >= strtotime($dt_trf['dt_min']) and strtotime($val) <= strtotime($dt_trf['dt_max']))
			{
				...
				(
					$col == 'dt_max'
					and $dt_srv['dt_min'] != '0000-00-00'
					and strtotime($dt_srv['dt_min']) < strtotime($dt_trf['dt_max'])
					and strtotime($val) > strtotime($dt_trf['dt_min'])
				) or (
					$col == 'dt_min'
					and $dt_srv['dt_max'] != '0000-00-00'
					and strtotime($dt_srv['dt_max']) > strtotime($dt_trf['dt_min'])
					and strtotime($val) < strtotime($dt_trf['dt_max'])
				)
			)
			{
				echo json_encode($txt->errdat3->$id_lng);
				return;
			}
		}
	}elseif($tab == 'cat_hbr')
	...
	{
		$dt_chm = ftc_ass(sel_quo(
			'id_chm, dt_min, dt_max',
			'cat_hbr_chm_trf INNER JOIN cat_hbr_chm_trf_ssn ON cat_hbr_chm_trf.id = cat_hbr_chm_trf_ssn.id_trf',
			'cat_hbr_chm_trf_ssn.id',
			$id)
		);
		if(
			($col == 'dt_max' and strtotime($dt_chm['dt_min']) > strtotime($val) and $val !='0000-00-00' and !$flg_y)
			or ($col == 'dt_min' and strtotime($dt_chm['dt_max']) < strtotime($val) and $dt_chm['dt_max'] != '0000-00-00')
		)
		{
			echo json_encode($txt->errdat->$id_lng);
			return;
		...
		$rq_trf = sel_whe(
			'cat_hbr_chm_trf_ssn.id, cat_hbr_chm_trf_ssn.dt_min, cat_hbr_chm_trf_ssn.dt_max',
			'cat_hbr_chm_trf INNER JOIN cat_hbr_chm_trf_ssn ON cat_hbr_chm_trf.id = cat_hbr_chm_trf_ssn.id_trf',
			'cat_hbr_chm_trf_ssn.id!='.$id.' AND id_chm = '.$dt_chm['id_chm']
		);
		while($dt_trf = ftc_ass($rq_trf))
		{
			if(strtotime($val) >= strtotime($dt_trf['dt_min']) and strtotime($val) <= strtotime($dt_trf['dt_max']))
			{
				...
				(
					$col == 'dt_max'
					and $dt_chm['dt_min'] != '0000-00-00'
					and strtotime($dt_chm['dt_min']) < strtotime($dt_trf['dt_max'])
					and strtotime($val) > strtotime($dt_trf['dt_min'])
				) or (
					$col == 'dt_min'
					and $dt_chm['dt_max'] != '0000-00-00'
					and strtotime($dt_chm['dt_max']) > strtotime($dt_trf['dt_min'])
					and strtotime($val) < strtotime($dt_trf['dt_max'])
				)
			)
			{
	...
	{
		$dt_rgm = ftc_ass(sel_quo(
			'id_rgm, dt_min, dt_max',
			'cat_hbr_rgm_trf INNER JOIN cat_hbr_rgm_trf_ssn ON cat_hbr_rgm_trf.id = cat_hbr_rgm_trf_ssn.id_trf',
			'cat_hbr_rgm_trf_ssn.id',
			$id)
		);
		if(
			($col == 'dt_max' and strtotime($dt_rgm['dt_min']) > strtotime($val) and $val !='0000-00-00' and !$flg_y)
			or ($col == 'dt_min' and strtotime($dt_rgm['dt_max']) < strtotime($val) and $dt_rgm['dt_max'] != '0000-00-00')
		)
		{
			echo json_encode($txt->errdat->$id_lng);
			return;
		...
		$rq_trf = sel_whe(
			'cat_hbr_rgm_trf_ssn.id, cat_hbr_rgm_trf_ssn.dt_min, cat_hbr_rgm_trf_ssn.dt_max',
			'cat_hbr_rgm_trf INNER JOIN cat_hbr_rgm_trf_ssn ON cat_hbr_rgm_trf.id = cat_hbr_rgm_trf_ssn.id_trf',
			'cat_hbr_rgm_trf_ssn.id!='.$id.' AND id_rgm', $dt_rgm['id_rgm']
		);
		while($dt_trf = ftc_ass($rq_trf))
		{
			if(
			...
			)
			{
				...
				(
					$col == 'dt_max'
					and $dt_rgm['dt_min'] != '0000-00-00'
					and strtotime($dt_rgm['dt_min']) < strtotime($dt_trf['dt_max'])
					and strtotime($val) > strtotime($dt_trf['dt_min'])
				) or (
					$col == 'dt_min'
					and $dt_rgm['dt_max'] != '0000-00-00'
					and strtotime($dt_rgm['dt_max']) > strtotime($dt_trf['dt_min'])
					and strtotime($val) < strtotime($dt_trf['dt_max'])
				)
			)
			{
