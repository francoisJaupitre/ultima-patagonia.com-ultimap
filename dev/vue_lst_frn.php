...
?>
	<li <?php if($flg_enter){echo 'id="enter_frn'.$id_dev_srv.'" style="background-color: Chocolate;"';} ?> onClick="maj('dev_srv','id_frn','0',<?php echo $id_dev_srv ?>);searchSrv(<?php echo $id_frn.',0,0,'.$id_dev_srv ?>);"><?php echo $txt->nodef->$id_lng; ?></li>
<?php
	$flg_enter = false;
}
if($id_frn>-1 and substr(upnoac($txt->libre->$id_lng),0,strlen($src))==$src){
?>
	<li <?php if($flg_enter){echo 'id="enter_frn'.$id_dev_srv.'" style="background-color: Chocolate;"';} ?> onClick="maj('dev_srv','id_frn','-1',<?php echo $id_dev_srv ?>);searchSrv(<?php echo $id_frn.','.$id_ctg_srv.','.$id_vll.','.$id_dev_srv ?>);"><?php echo $txt->libre->$id_lng; ?></li>
<?php
	$flg_enter = false;
}
...
?>
	<li <?php if($flg_enter){echo 'id="enter_frn'.$id_dev_srv.'" style="background-color: Chocolate;"';} ?>>
		...
		<ul id="ul_srv_frn<?php echo $id_dev_srv.'_'.$dt_lst_frn['id']; ?>" class="cmd_srv<?php echo $id_dev_srv; ?>" style="display: none">
			<li onClick="maj('dev_srv','id_frn',<?php echo $dt_lst_frn['id'].','.$id_dev_srv ?>); searchSrv(<?php echo $dt_lst_frn['id'].','.$id_ctg_srv.','.$id_vll.','.$id_dev_srv ?>);"><?php echo $txt->ajt->$id_lng ?></li>
