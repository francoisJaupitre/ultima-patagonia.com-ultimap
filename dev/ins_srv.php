<?php
$dt_cat_srv = ftc_ass(sel_quo("*", "cat_srv", "id", $id_cat_srv));
if($dt_cat_srv['res'] == 1)
{
	$resrv = 0;
}else{
...
if(!empty($dt_cat_srv['alerte']))
{
	$alt .= $dt_cat_srv['nom'].' : '.$dt_cat_srv['alerte'].",\n";
}
$bss = array();
$err_bss = '';
...
if(!is_null($rq_bss) and num_rows($rq_bss) > 0)
{
	...
	if($flg_trf)
	{
		...
		upd_quo('dev_srv', array('dt_min', 'dt_max', 'id_frn'), array($dt_min, $dt_max, $dt_trf['id_frn']), $id_dev_srv);
	}
	if($err_bss != '')
	{
