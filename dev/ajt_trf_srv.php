...
while($dt_jrn = ftc_ass($rq_jrn)){
	...
	while($dt_prs = ftc_ass($rq_prs)){
		...
		$rq_srv = select("id,id_cat,nom,crr","dev_srv","id_prs",$id_dev_prs);
		while($dt_srv = ftc_ass($rq_srv)){
			...
			if($id_cat_srv!=0){
				...
				if($date!='0000-00-00'){$err_srv .= $dt_dev_srv['nom'].' '.$txt->jour->$id_lng.' '.date("d/m/Y",strtotime($date))."\n";}
				else{$err_srv .= $dt_dev_srv['nom'].' '.$txt->jour->$id_lng.' '.$ord_jrn."\n";}
			}
			if($err_bss!=''){$err_srv .= $dt_srv['nom'].' '.$txt->jour->$id_lng.' '.$ord_jrn."\n";}
		}
	}
}
?>
