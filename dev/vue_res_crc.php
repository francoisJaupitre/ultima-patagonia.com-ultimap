...
if(isset($_POST['id_dev_crc'])){
	...
	include("../cfg/frn.php");
	include("../cfg/vll.php");
	$dt_crc = ftc_ass(sel_quo("sgl, dbl_mat, dbl_twn, tpl_mat, tpl_twn, qdp, ptl, psg, crr", "dev_crc", "id", $id_dev_crc));
...
if($vue_res and isset($mdl_datas))
{
	foreach($mdl_datas as $id_dev_mdl => $dt_mdl)
	{
		...
		while($dt_jrn = ftc_ass($rq_jrn))
		{
			...
			while($dt_prs = ftc_ass($rq_prs))
			{
				...
				if(($dt_prs['opt'] and $cnf < 1) or ($dt_prs['res'] == 1 or $dt_prs['res'] == -1 and $cnf > 0))
				{
					$id_cat_prs = $dt_prs['id_cat'];
				}
				$rq_srv = sel_quo("*", "dev_srv", array("opt", "id_prs"), array(1, $dt_prs['id']));
				while($dt_srv = ftc_ass($rq_srv))
				{
					$frs_srv = 1 + $dt_srv['frs'];
					if($dt_prs['res'] == 1 or $dt_prs['opt'])
					{//IF PAID THEN CANCELLED
						if($dt_prs['res'] != 1 and $cnf > 0 and $dt_prs['opt'])
						{
							$id_frn = -1;
						}else{
						...
						if(!isset($lst_frn) or !in_array($id_frn, $lst_frn))
						{
							...
							if($id_frn > 0)
							{
								$dt_frn = ftc_ass(sel_quo("frs, wap", "cat_frn", "id", $id_frn));
								$frs_frn = 1 + $dt_frn['frs'];
								$frn_wap[$id_frn] = $dt_frn['wap'];
							}else{
								$frs_frn = $frs_srv;
								$frn_wap[$id_frn] = 0;
							}
						...
						$rq_srv_pay = sel_quo("*", "dev_srv_pay", "id_srv", $dt_srv['id']);
						while($dt_srv_pay = ftc_ass($rq_srv_pay))
						{
							if(!$dt_srv_pay['pay'])
							{
								if($cfg_crr_sp[$dt_srv_pay['crr']] == 1)
								{
									$liq_frn[$id_frn] += $dt_srv_pay['liq'] * $cfg_crr_tx[$dt_srv_pay['crr']] * $frs_frn;
								}else{
									$liq_frn[$id_frn] += $dt_srv_pay['liq'] / $cfg_crr_tx[$dt_srv_pay['crr']] * $frs_frn;
								}
								if($dt_srv_pay['date'] != '0000-00-00')
								{
									$dat_frn[$id_frn][] = $dt_srv_pay['date'];
								}
							}else{
								if($dt_srv_pay['sup'])
								{
									$liq_frn[$id_frn] += $dt_srv_pay['liq'] * $dt_srv_pay['taux'] * $frs_frn;
								}else{
									$liq_frn[$id_frn] += $dt_srv_pay['liq'] / $dt_srv_pay['taux'] * $frs_frn;
								}
							}
						...
						if($dt_prs['opt'])
						{
							$sup = $dt_srv['sup'];
							$taux = $dt_srv['taux'];
							$dt_srv_trf = ftc_ass(sel_quo("trf_net", "dev_srv_trf", array("id_srv", "base"), array($dt_srv['id'], $bss)));
							$cot_sr = $dt_srv_trf['trf_net'] * $bss * $frs_srv;
							if(!$sup)
							{
								if($sup_dev)
								{
									$cot_srv[$id_frn] += $cot_sr * $taux * $taux_dev;
								}elseif($taux_dev != 0)
								{
									$cot_srv[$id_frn] += $cot_sr * $taux / $taux_dev;
								}
				...
				if($cnf > 0)
				{
					$rq_hbr = sel_quo("*", "dev_hbr", "id_prs", $dt_prs['id'], "sel DESC");
				}else{
					$rq_hbr = sel_quo("*", "dev_hbr", "id_prs", $dt_prs['id'], "opt DESC");
				}
				while($dt_hbr = ftc_ass($rq_hbr))
				{
					$frs_hbr = 1 + $dt_hbr['frs'];
					if(($cnf > 0 or ($cnf < 1 and $dt_prs['opt'])) and $dt_hbr['id_cat'] > -2)
					{
						if(($dt_prs['res'] == 1 and $dt_hbr['sel'] and $cnf > 0) or ($dt_hbr['opt'] and $cnf < 1))
						{
							if($dt_hbr['id_cat'] > 0)
							{
							...
							if($id_hbr < 0 or $id_chm != -2)
							{
								$res_hbr[$id_hbr][$id_chm][] = $dt_hbr['res'];
							}
							if(($dt_prs['res'] == 1 and $cnf > 0) or ($dt_prs['opt'] and $cnf < 1))
							{
								$rq_hbr_pay = sel_quo("*", "dev_hbr_pay", "id_hbr", $dt_hbr['id']);
								while($dt_hbr_pay = ftc_ass($rq_hbr_pay))
								{
									if(!$dt_hbr_pay['pay'])
									{
										if($cfg_crr_sp[$dt_hbr_pay['crr']] == 1)
										{
											$liq_hbr[$id_hbr][$id_chm] += $dt_hbr_pay['liq'] * $cfg_crr_tx[$dt_hbr_pay['crr']] * $frs_hbr;
										}else{
											$liq_hbr[$id_hbr][$id_chm] += $dt_hbr_pay['liq'] / $cfg_crr_tx[$dt_hbr_pay['crr']] * $frs_hbr;
										}
										if($dt_hbr_pay['date'] != '0000-00-00')
										{
											$dat_hbr[$id_hbr][$id_chm][] = $dt_hbr_pay['date'];
										}
									}else{
										if($dt_hbr_pay['sup'])
										{
											$liq_hbr[$id_hbr][$id_chm] += $dt_hbr_pay['liq'] * $dt_hbr_pay['taux'] * $frs_hbr;
										}else{
											$liq_hbr[$id_hbr][$id_chm] += $dt_hbr_pay['liq'] / $dt_hbr_pay['taux'] * $frs_hbr;
										}
									}
						...
						}elseif(($dt_prs['res'] == 1 and !$dt_hbr['sel'] and $cnf > 0) or (!$dt_hbr['opt'] and $cnf < 1))
						{
							if($dt_hbr['id_cat'] > 0)
							{
								$id_hbr_opt = $dt_hbr['id_cat'];
							}else{
								$id_hbr_opt = -$dt_hbr['id'];
							}
							if($dt_hbr['id_cat_chm'] > 0)
							{
								$id_chm_opt = $dt_hbr['id_cat_chm'];
							}else{
								$id_chm_opt = -$dt_hbr['id'];
							}
							if(!isset($cot_hbr_opt[$id_hbr_opt][$id_chm_opt]))
							{
								$cot_hbr_opt[$id_hbr_opt][$id_chm_opt] = 0;
							}
							$flg_hbr_opt = true;
							$flg_chm_opt = true;
							if(isset($hbr_opt))
							{
								foreach($hbr_opt as $hb_opt)//REMPLACER PAR IN_ARRAY & ARRAY_SEARCH
								{
									if($hb_opt == $id_hbr_opt)
									{
										...
										{
											foreach($chm_hbr_opt[$hb_opt] as $chm_opt) //REMPLACER PAR IN_ARRAY & ARRAY_SEARCH
											{
												if($chm_opt == $id_chm_opt)
												{
													$flg_chm_opt = false;
												}
											}
							...
							if($flg_chm_opt)
							{
								$chm_hbr_opt[$id_hbr_opt][] = $id_chm_opt;
							}
							...
							if($id_hbr_opt < 0 or $id_chm_opt != 2)
							{
								$res_hbr_opt[$id_hbr_opt][$id_chm_opt][] = $dt_hbr['res'];
							}
						}
					}
					if($dt_prs['opt'] and $dt_hbr['id_cat'] > -2)
					{
						if($dt_hbr['opt'])
						{
							if($dt_prs['res'] == 1 or $cnf < 1)
							{
								...
								$cot_hb = (
									$dt_hbr['db_net_chm'] * ($dbl_mat + $dbl_twn)
									+ $dt_hbr['sg_net_chm'] * ($sgl + $psg)
									+ $dt_hbr['tp_net_chm'] * ($tpl_mat + $tpl_twn)
									+ $dt_hbr['qd_net_chm'] * $qdp
								) * $frs_hbr;
								if(!$sup)
								{
									if($sup_dev)
									{
										$cot_hbr[$id_hbr][$id_chm] += $cot_hb * $taux * $taux_dev;
									}elseif($taux_dev != 0)
									{
										$cot_hbr[$id_hbr][$id_chm] += $cot_hb * $taux / $taux_dev;
									}
								...
								{
									$cot_hb = (
										$dt_hbr['db_net_rgm'] * ($dbl_mat + $dbl_twn)
										+ $dt_hbr['sg_net_rgm'] * ($sgl + $psg)
										+ $dt_hbr['tp_net_rgm'] * ($tpl_mat + $tpl_twn)
										+ $dt_hbr['qd_net_rgm'] * $qdp
									) * $frs_hbr;
									if(!$sup)
									{
										if($sup_dev)
						...
						}elseif($cnf < 1)
						{
							...
							if(!$sup)
							{
								if($sup_dev)
								{
									$cot_hbr_opt[$id_hbr_opt][$id_chm_opt] += $cot_hb * $taux * $taux_dev;
								}elseif($taux_dev != 0)
								{
									$cot_hbr_opt[$id_hbr_opt][$id_chm_opt] += $cot_hb * $taux / $taux_dev;
								}
	...
	if(isset($lst_frn))
	{
		...
		if($cnf > 0)
		{
			...
			{
				foreach($res_frn[$fr] as $id_res_frn)
				{
...
<?php
			foreach($jr_frn[$fr] as $i => $jr)
			{
?>
						<span class="lnk" onclick="scroll2(<?php echo $jr_frn_id[$fr][$i].','.$mdl_frn_id[$fr][$i]; ?>)">
<?php
				echo $jr;
?>
						</span> -
...
<?php
			...
			{
?>
							class="lnk inf<?php echo $fr ?>frn"
							onmouseover="vue_elem('inf', <?php echo $fr ?>, 'frn')"
							onclick="vue_cmd('vue_cmd_frn<?php echo $fr; ?>')"
<?php
			}
?>
						>
<?php
			if($fr > 0)
			{
				echo stripslashes($frn[$fr]);
			}elseif($fr == 0)
			{
				echo $txt->nodef->$id_lng;
			}else{
				echo $txt->cotnocnf->$id_lng;
			}
?>
						</span>
						<div
							id="vue_cmd_frn<?php echo $fr; ?>"
							class="cmd mw200 wsn"
							style="text-align:left;left:50%;transform:translate(-50%);"
						>
...
<?php
			...
			{
?>
								<li onclick="window.open('https://wa.me/<?php echo $frn_wap[$fr]; ?>', '_blank')"><?php echo $txt->wap->$id_lng; ?></li>
<?php
			}
?>
...
?>
					...
					<td
						...
						style="
<?php
			if($cnf > 0 and $liq_frn[$fr] <= 0 and $fr >= 0)
			{
...
?>
					...
					>
<?php
			echo number_format($liq_frn[$fr], $prm_crr_dcm[1], '.', '');
?>
					</td>
<?php
			if($fr != 0)
...
?>
					<td
						...
						style="
<?php
					if($cot_srv[$fr] - $liq_frn[$fr] < 0)
					{
						echo 'background-color: tomato;';
					}
					if($cot_srv[$fr] - $liq_frn[$fr] > 0 and $liq_frn[$fr] != 0)
					{
...
<?php
				...
				{
					echo '<td class="stl1" style="background-color: tomato;"></td>';
				}else{
...
?>
					...
					>
<?php
		echo number_format($cot, $prm_crr_dcm[1], '.', '')
?>
					</td>
<?php
		if($cnf > 0)
...
?>
					...
					>
<?php
			echo number_format($liq, $prm_crr_dcm[1], '.', '')
?>
					</td>
					<td
						class="stl1"
...
?>
					...
					>
<?php
			echo number_format($dif, $prm_crr_dcm[1], '.', '')
?>
					</td>
					<td></td>
<?php
	...
	if(isset($hbr))
	{
		...
		if($cnf > 0)
		{
			foreach($hbr as $hb)
			{
				...
				{
					foreach($res_hbr[$hb][$chm] as $id_res_chm)
					{
...
<?php
		foreach($hbr as $hb)
		{
			if($hb > 0)
			{
				$dt_hbr = ftc_ass(sel_quo("nom, id_vll, id_frn, wap", "cat_hbr", "id", $hb));
				$id_vll = $dt_hbr['id_vll'];
				$hbr_nom = $dt_hbr['nom'];
				if($dt_hbr['id_frn'] > 0)
				{
					$dt_frn = ftc_ass(sel_quo("wap", "cat_frn", "id", $dt_hbr['id_frn']));
					$hbr_wap = $dt_frn['wap'];
				}else{
					$hbr_wap = $dt_hbr['wap'];
				}
			...
			{
				$h = -$hb;
				$dt_hbr = ftc_ass(sel_quo("nom, id_vll, nom_chm", "dev_hbr", "id", $h));
				$id_vll = $dt_hbr['id_vll'];
				$hbr_nom = $dt_hbr['nom'];
				$hbr_wap = 0;
			}else{
			...
			foreach($chm_hbr[$hb] as $chm)
			{
				if($hb >= 0)
				{
					if($chm > 0)
					{
						$dt_chm = ftc_ass(sel_quo("nom", "cat_hbr_chm", "id", $chm));
						$chm_nom = $dt_chm['nom'];
					}elseif($chm == -1)
					{
						$chm_nom = $txt->nodef->$id_lng;
					}elseif($chm == -2)
					{
						$chm_nom = $txt->libre->$id_lng;
					}else{
						$chm_nom = '';
					}
				}else{
					$chm_nom = $dt_hbr['nom_chm'];
				}
...
?>
				<tr style="font-weight: normal; <?php echo $color; ?>">
					<td class="stl1" style="padding: 0px 5px;">
<?php
				foreach($jr_hbr[$hb][$chm] as $i => $jr)
				{
?>
						<span class="lnk" onclick="scroll2(<?php echo $jr_hbr_id[$hb][$chm][$i].', '.$mdl_hbr_id[$hb][$chm][$i]; ?>)">
<?php
					echo $jr;
?>
						</span> -
...
?>
					...
					<td class="stl1" style="padding: 0px 5px; <?php if($dif_hbr[$hb] == 2){echo "background-color: mediumorchid;";} ?>">
<?php
				if($id_vll > 0)
				{
...
<?php
				if($dif_hbr[$hb] > 0)
				{
					echo 'background-color: mediumorchid;';
				}
?>
						"
					>
...
<!--COMMANDES-->
						<span
							class="lnk inf<?php echo $hb ?>hbr"
							onmouseover="vue_elem('inf', <?php echo $hb ?>, 'hbr')"
							onclick="
<?php
					if($chm > 0)
					{
						echo "vue_cmd('vue_cmd_hbr".$hb."_".$chm."')";
					}else{
						echo "window.parent.opn_frm('cat/ctr.php?cbl=hbr&id=".$hb."')";
					}
?>
							"
						>
...
?>
						<div
							id="vue_cmd_hbr<?php echo $hb.'_'.$chm; ?>"
							class="cmd mw200 wsn"
							style="text-align:left; left:50%; transform:translate(-50%);"
						>
							<strong><?php echo $txt->cmd->$id_lng; ?></strong>
							<ul>
								<li
									onclick="
										window.parent.opn_frm('cat/ctr.php?cbl=hbr&id=<?php echo $hb;?>');
										document.getElementById('vue_cmd_hbr<?php echo $hb.'_'.$chm; ?>').style.display='none';
									"
								>
<?php
						echo $txt->cat->$id_lng;
?>
								...
								<li
									onclick="
										window.open('../resources/php/docxHbr.php?id=<?php echo $id_dev_crc;?>&hbr=<?php echo $hb;?>&chm=<?php echo $chm;?>');
									"
								>
<?php
						echo $txt->lst_res->$id_lng;
?>
								...
								<li
									onclick="
										window.open('../resources/php/vchHbr.php?id=<?php echo $id_dev_crc;?>&hbr=<?php echo $hb;?>');
										"
									>
<?php
						echo $txt->vch->$id_lng;
...
<?php
						...
						{
?>
								<li onclick="searchHbr(<?php echo $hb.', '.$chm ?>, 0, 0, 0, 0, 'updateRates');">
<?php
							echo $txt->acttrf->$id_lng;
?>
								</li>
<?php
						}
						...
						{
?>
							<li onclick="window.open('https://wa.me/<?php echo $hbr_wap; ?>', '_blank')"><?php echo $txt->wap->$id_lng; ?></li>
<?php
						}
?>
							</ul>
...
?>
					...
					<td class="stl1" style="padding: 0px 5px; <?php if($dif_chm[$chm] == 1){echo "background-color: mediumorchid;";} ?>">
<?php
						echo stripslashes($chm_nom);
?>
					</td>
<?php
				if($cnf > 0 or $flg_res)
...
<?php
				if($chm > -2)
				{
						echo number_format($cot_hbr[$hb][$chm], $prm_crr_dcm[1], '.', '');
				}
?>
					</td>
					<td
						...
						style="
<?php
				if($chm > -2 and $cnf > 0 and $liq_hbr[$hb][$chm] <= 0)
				{
					echo 'background-color: tomato;';
				}
?>
					...
					>
<?php
				if($chm > -2 or $liq_hbr[$hb][$chm] != 0)
				{
					echo number_format($liq_hbr[$hb][$chm], $prm_crr_dcm[1], '.', '');
				}
?>
					</td>
<?php
...
?>
					<td
						...
						style="
<?php
					if($chm > -2 and number_format($cot_hbr[$hb][$chm]-$liq_hbr[$hb][$chm], $prm_crr_dcm[1], '.', '') < 0)
					{
						echo 'background-color: tomato;';
					}
					if(
						$chm > -2
						and number_format($cot_hbr[$hb][$chm]-$liq_hbr[$hb][$chm], $prm_crr_dcm[1], '.', '') > 0
						and $liq_hbr[$hb][$chm] != 0
					)
					{
						echo 'background-color: skyblue;';
					}
...
<?php
					...
					{
						echo '<td style="background-color: tomato;"></td>';
					}else{
						echo '<td class="stl1"></td>';
					}
				}else{
...
?>
					...
					>
<?php
					if($chm > -2 and is_array($dat_hbr[$hb][$chm]))
					{
						echo date("d/m/Y", strtotime($dat));
					}
?>
	...
	if(isset($hbr_opt))
	{
		...
		if($cnf > 0)
		{
			foreach($hbr_opt as $hb_opt)
			{
				...
				{
					foreach($res_hbr_opt[$hb_opt][$chm_opt] as $id_res_chm)
					{
						$color_chm[] = $id_res_chm;
					}
				}
...
<?php
		foreach($hbr_opt as $hb_opt)
		{
			if($hb_opt != 0)
			{
				$dt_hbr = ftc_ass(sel_quo("nom, id_vll, id_frn, wap", "cat_hbr", "id", $hb_opt));
				$id_vll = $dt_hbr['id_vll'];
				$hbr_nom = $dt_hbr['nom'];
				if($dt_hbr['id_frn'] > 0)
				{
			...
			{
				if($chm_opt > 0){
					$dt_chm = ftc_ass(sel_quo("nom", "cat_hbr_chm", "id", $chm_opt));
					$chm_nom = $dt_chm['nom'];
				}elseif($chm_opt == -1)
				{
					$chm_nom = $txt->nodef->$id_lng;
...
?>
				<tr style="font-weight: normal; <?php echo $color; ?>">
					<td class="stl1" style="padding: 0px 5px;">
<?php
				foreach($jr_hbr_opt[$hb_opt][$chm_opt] as $i => $jr)
				{
?>
						<span
							class="lnk"
							onclick="scroll2(<?php echo $jr_hbr_id_opt[$hb_opt][$chm_opt][$i].', '.$mdl_hbr_id_opt[$hb_opt][$chm_opt][$i]; ?>)"
						>
<?php
					echo $jr;
?>
...
<!--COMMANDES-->
						<span
							class="lnk inf<?php echo $hb_opt ?>hbr"
							onmouseover="vue_elem('inf', <?php echo $hb_opt ?>, 'hbr')"
							onclick="vue_cmd('vue_cmd_hbr_opt<?php echo $hb_opt.'_'.$chm_opt; ?>')"
						>
<?php
				echo stripslashes($hbr_nom)
?>
						...
						<div
							id="vue_cmd_hbr_opt<?php echo $hb_opt.'_'.$chm_opt; ?>"
							class="cmd mw200 wsn"
							style="text-align:left; left:50%; transform:translate(-50%);"
						>
							<strong><?php echo $txt->cmd->$id_lng; ?></strong>
							<ul>
								<li
									onclick="
										window.parent.opn_frm('cat/ctr.php?cbl=hbr&id=<?php echo $hb_opt;?>');
										document.getElementById('vue_cmd_hbr_opt<?php echo $hb_opt.'_'.$chm_opt; ?>').style.display='none';
									"
								>
<?php
				echo $txt->cat->$id_lng;
?>
								...
								<li
									onclick="
										window.open('../resources/php/docxHbr.php?id=<?php echo $id_dev_crc;?>&hbr=<?php echo $hb_opt;?>&chm=<?php echo $chm_opt;?>');
									">
<?php
				echo $txt->lst_res->$id_lng;
?>
								</li>
								<li onclick="mailHbr(<?php echo $hb_opt.', '.$chm_opt.', 0'; ?>);">
<?php
				echo $txt->maildevis->$id_lng;
?>
								</li>
<?php
				if($aut['res'])
...
<?php
				...
				{
?>
								<li onclick="searchHbr(<?php echo $hb_opt.', '.$chm_opt ?>, 0, 0, 0, 0, 'updateRates');">
<?php
					echo $txt->acttrf->$id_lng;
?>
								</li>
								<li onclick="searchHbr(<?php echo $hb_opt.', '.$chm_opt ?>, 0, 0, 0, 0, 'sup');">
<?php
					echo $txt->sup->$id_lng;
?>
								</li>
...
<?php
					if($chm_opt > -2)
					{
						echo number_format($cot_hbr_opt[$hb_opt][$chm_opt], $prm_crr_dcm[1], '.', '');
					}
?>
					</td>
<?php
