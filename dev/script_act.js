...
function mel_frn(id_res_frn){
	...
	xmlhttp.onreadystatechange=function(){
		if(xmlhttp.readyState==4){
			if(xmlhttp.status==200){
				...
				if(id_res_frn==0){window.parent.act_frm('crc_res_srv'+id_dev_crc);window.parent.act_frm('crc_res_frn'+id_dev_crc);}
				else{window.parent.act_frm('frn_ope_frn'+id_res_frn);window.parent.act_frm('frn_ope_srv'+id_res_frn);}
				window.parent.act_frm('frn_ope');
				alt(rsp_mel[0]);
			}
			else if(xmlhttp.status==408){mel_frn(id_res_frn);}
	...
	xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xmlhttp.send("id_dev_crc="+id_dev_crc+"&id_res_frn="+id_res_frn);
}

function mel_hbr(id_res_hbr,id_res_chm){
	if(window.XMLHttpRequest){xhttp=new XMLHttpRequest();}
	else{xhttp=new ActiveXObject("Microsoft.XMLHTTP");}
	xhttp.open("GET","../fct/txt_js.xml",false);
	xhttp.send();
	...
	xmlhttp.onreadystatechange=function(){
		if(xmlhttp.readyState==4){
			if(xmlhttp.status==200){
    			...
    			link.click();
					for(var j = 2;j < rsp_hbr.length;j++){vue_elem('hbr_res'+rsp_hbr[j],rsp_hbr[j]);}
				}
				document.body.removeChild(link);
				if(id_res_hbr==0){window.parent.act_frm('crc_res_hbr'+id_dev_crc);}
				else{window.parent.act_frm('cat_res_hbr'+id_res_hbr);}
				window.parent.act_frm('hbr_ope');
				alt(rsp_mel[0]);
			}
			else if(xmlhttp.status==408){mel_hbr(id_res_hbr,id_res_chm);}
	...
	xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	xmlhttp.send("id_dev_crc="+id_dev_crc+"&id_res_hbr="+id_res_hbr+"&id_res_chm="+id_res_chm);
}
