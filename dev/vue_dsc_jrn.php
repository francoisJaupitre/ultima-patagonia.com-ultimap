...
<tr>
	...
	<td class="<?php if($opt_jrn){echo 'lcrl';} else{echo 'tht';} ?>" style="width:100%; text-align: center;">
		<div style="margin-right: 5px; position: relative" onload="alt('load')">
			...
			<div id="jrn_dsc<?php echo $id_dev_jrn ?>" class="ust rich rich_dt_crc rich_dt_mdl<?php echo $id_dev_mdl ?> rich_dsc_jrn<?php echo $id_dev_jrn ?>" style="height: 80px; color: #<?php echo $col[$id_col_mdl]; ?>" <?php if(($aut['dev'] and $cnf<1) or ($aut['res'] and $cnf>0)){ ?> onclick="richtext(this.id,'dev_jrn','dsc',<?php echo $id_dev_jrn ?>);this.onclick=null;" <?php } ?> ><?php echo nl2br(stripslashes($dsc_jrn)) ?></div>
