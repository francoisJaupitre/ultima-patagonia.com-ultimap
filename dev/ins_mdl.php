<?php
$dt_mdl = ftc_ass(sel_quo("*","cat_mdl LEFT JOIN cat_mdl_txt ON cat_mdl.id = cat_mdl_txt.id_mdl AND lgg = ".$id_lgg,"cat_mdl.id",$id_cat_mdl));
$id_dev_mdl = insert("dev_mdl",array("id_crc","id_cat","nom","ord","titre","dsc","col","fus","vue_dbl","trf"),array($id_dev_crc,$id_cat_mdl,$dt_mdl['nom'],$ord_mdl,$dt_mdl['titre'],$dt_mdl['dsc'],1,$fus,1,$trf_mdl));
if(isset($obj) and $obj == 'mdl' and !empty($dt_mdl['sel_mdl_jrn'])) {$sel_mdl_jrn = explode(",",$dt_mdl['sel_mdl_jrn']);}
else{unset($sel_mdl_jrn);}
if(empty($dt_mdl['titre'])) {$err_mdl .= $dt_mdl['nom'].",\n";}
if(!empty($dt_mdl['alerte'])) {$alt .= $dt_mdl['nom'].' : '.$dt_mdl['alerte'].",\n";}
$rq_rgn = sel_quo("id_rgn","cat_mdl_rgn","id_mdl",$id_cat_mdl,"","DISTINCT");
while($dt_rgn = ftc_ass($rq_rgn)) {insert("dev_mdl_rgn",array("id_mdl","id_rgn"),array($id_dev_mdl,$dt_rgn['id_rgn']));}
$ir_rmn = 0;
$opt_jrn = 1;
$flg_sel = false;
...
while($dt_mdl_jrn = ftc_ass($rq_mdl_jrn)) {
	if(
		...
		$opt_jrn = 1;
		if($dt_mdl_jrn['opt']) {$flg_sel = false;}
