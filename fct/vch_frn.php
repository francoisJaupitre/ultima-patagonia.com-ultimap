		...
		foreach($tab_frn[$id_dev] as $i => $fr){
			foreach(array_unique($tab_prs[$id_dev][$fr]) as $j => $pr){
				if($tab_ctg_prs[$id_dev][$fr][$j]!=19 and $tab_ctg_prs[$id_dev][$fr][$j]!=20 and $tab_ctg_prs[$id_dev][$fr][$j]!=24){
					...
					if(!empty($dt_cat_frn['titre'])){$section->addText(stripslashes(replace($dt_cat_frn['titre'])), $fontStyle3, $paragraphStyle2);}
					else{$section->addText(stripslashes(replace($dt_cat_frn['nom'])), $fontStyle3 , $paragraphStyle2);}
					if(!empty($dt_cat_frn['adresse'])){$section->addText(stripslashes(replace($dt_cat_frn['adresse'])), $fontStyle, $paragraphStyle2);}
					if(!empty($dt_cat_frn['tel'])){$section->addText(stripslashes(replace($dt_cat_frn['tel'])), $fontStyle, $paragraphStyle2);}
					...
					if($tab_rva[$id_dev][$fr][$j]!=''){
						...
						if($id_lgg_frn!=$id_lgg){$cell->addText('['.stripslashes(replace($txt_vch->rva->$id_lgg_frn.']')), $fontStyle, $paragraphStyle2);}
					}
					$cell = $table->addCell(2000, $cellStyle);
					...
					if($id_lgg_frn!=$id_lgg){$cell->addText('['.stripslashes(replace($txt_vch->sir->$id_lgg_frn.']')), $fontStyle, $paragraphStyle2);}
					$cell = $table->addCell(3500, $cellStyle);
					$msg = $nom_gpe[$id_dev].' x'.$tab_nb_pax_fr[$id_dev][$fr][$j];
					if($prs_guia[$tab_prs_id[$id_dev][$fr][$j]]){$msg .= ' +'.$txt_vch->guia->$id_lgg_frn;}
					$cell->addText(stripslashes(replace($msg)), $fontStyle3, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$dt_cat_prs = ftc_ass(sel_quo("cat_prs.*,cat_prs_txt.titre","cat_prs LEFT JOIN cat_prs_txt ON cat_prs_txt.id_prs = cat_prs.id AND lgg=".$lgg_crc[$id_dev],"cat_prs.id",$pr));
					if(!empty($dt_cat_prs['titre'])){$section->addText(stripslashes(replace($dt_cat_prs['titre'])), $fontStyle3, $paragraphStyle2);}
					else{$section->addText(stripslashes(replace($dt_cat_prs['nom'])), $fontStyle3, $paragraphStyle2);}
					if($lgg_frn!=$lgg_crc[$id_dev]){
						$dt_cat_prs_trad = ftc_ass(sel_quo("titre","cat_prs_txt",array("lgg","id_prs"),array($lgg_frn,$pr)));
						if(!empty($dt_cat_prs_trad['titre'])){$section->addText('['.stripslashes(replace($dt_cat_prs_trad['titre'])).']', $fontStyle, $paragraphStyle2);}
					}
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					if($tab_ctg_prs[$id_dev][$fr][$j]!=10){
						...
						$msg = date("d/m/Y", strtotime($tab_in[$id_dev][$fr][$j]));
						if($tab_out[$id_dev][$fr][$j] != $tab_in[$id_dev][$fr][$j]){$msg .= ' - '.date("d/m/Y", strtotime($tab_out[$id_dev][$fr][$j]));}
						$section->addText(stripslashes(replace($msg)), $fontStyle4, $paragraphStyle2);
						$section->addText('', $fontStyle, $paragraphStyle2);
						$section->addText('', $fontStyle, $paragraphStyle2);
						if(!is_null($tab_hre[$id_dev][$fr][$j])){$section->addText(date("H:i", strtotime($tab_hre[$id_dev][$fr][$j])).$txt_vch->hs->$id_lgg, $fontStyle, $paragraphStyle2);}
						$rq_srv = sel_quo("id_cat,id_frn,ctg,nom,titre","dev_srv LEFT JOIN cat_srv_txt ON dev_srv.id_cat = cat_srv_txt.id_srv AND cat_srv_txt.lgg=".$lgg_frn,array("id_frn","id_prs"),array($fr,$tab_prs_id[$id_dev][$fr][$j]),"id_frn,ctg,nom");
						while($dt_srv = ftc_ass($rq_srv)){
							$msg = '';
							if($mrk_ctg_ctg_srv[$dt_srv['ctg']]){$msg .= ' '.$ctg_srv[$id_lgg_frn][$dt_srv['ctg']];}
							if($mrk_nom_ctg_srv[$dt_srv['ctg']]){
								if($dt_srv['titre']==''){$msg .= ' '.$dt_srv['nom'];}
								else{$msg .= ' '.$dt_srv['titre'];}
							}
							if($msg!=''){$section->addText(stripslashes(replace($msg)), $fontStyle, $paragraphStyle2);}
						}
					}
					else{
						$flg_pck = true;
						for($k=$j-1; $k>=0 ; $k--){
							if($tab_ctg_prs[$id_dev][$fr][$k]==19 and $flg_pck){
								...
								if(!is_null($tab_hre[$id_dev][$fr][$k])){$msg .= ' - '.date("H:i", strtotime($tab_hre[$id_dev][$fr][$k])).$txt_vch->hs->$id_lgg;}
								$section->addText(stripslashes(replace($msg)), $fontStyle4, $paragraphStyle2);
								$dt_cat_prs = ftc_ass(sel_quo("cat_prs.*,cat_prs_txt.titre","cat_prs LEFT JOIN cat_prs_txt ON cat_prs_txt.id_prs = cat_prs.id AND lgg=".$lgg_crc[$id_dev],"cat_prs.id",$tab_prs[$id_dev][$fr][$k]));
								if(!empty($dt_cat_prs['titre'])){$section->addText(stripslashes(replace($dt_cat_prs['titre'])), $fontStyle, $paragraphStyle);}
								else{$section->addText(stripslashes(replace($dt_cat_prs['nom'])), $fontStyle, $paragraphStyle);}
								if($lgg_frn!=$lgg_crc[$id_dev]){
									$dt_cat_prs_trad = ftc_ass(sel_quo("titre","cat_prs_txt",array("lgg","id_prs"),array($lgg_frn,$tab_prs[$id_dev][$fr][$k])));
									if(!empty($dt_cat_prs_trad['titre'])){$section->addText('['.stripslashes(replace($dt_cat_prs_trad['titre'])).']', $fontStyle, $paragraphStyle);}
								}
								$section->addText('', $fontStyle, $paragraphStyle2);
								$section->addText('', $fontStyle, $paragraphStyle2);
								$section->addText('', $fontStyle, $paragraphStyle2);
					...
					if($dt_mdl['trf']){$rq_lst_pax = sel_quo("*","dev_mdl_pax INNER JOIN grp_pax ON dev_mdl_pax.id_pax = grp_pax.id","id_mdl",$tab_mdl[$id_dev][$fr][$j],"ord");}
					else{$rq_lst_pax = sel_quo("*","dev_crc_pax INNER JOIN grp_pax ON dev_crc_pax.id_pax = grp_pax.id","id_crc",$id_dev,"ord");}
					$section->addText('', $fontStyle, $paragraphStyle2);
					$section->addText('', $fontStyle, $paragraphStyle2);
					$flg_pax = true;
