<?php
if(isset($_POST['id_dev_crc']) and isset($_POST['id_res_hbr']) and isset($_POST['id_res_chm'])) {
	$id_dev_crc = $_POST['id_dev_crc'];
	$id_res_hbr = $_POST['id_res_hbr'];
	$id_res_chm = $_POST['id_res_chm'];
	$obj='mail';
	include("res_hbr.php");
			...
			if(isset($tab_hbr[$id_dev])) {
				foreach($tab_hbr[$id_dev] as $i => $hb) {
					...
					if($flg_send[$id_dev][$hb]) {
						...
						$msg .= 'Saludos y gracias de antemano.</span>';
						...
						$nb++;
						foreach($lst_srv[$id_dev][$hb] as $id_srv) {
