<?php
include("../prm/fct.php");
$id_dev_srv_ctg = $_POST['id_dev_srv_ctg'];
$id_dev_srv_vll = $_POST['id_dev_srv_vll'];
$id_dev_srv = $_POST['id_dev_srv'];
$id_dev_crc = $_POST['id_dev_crc'];
$id_frn = $_POST['id_frn'];
if($id_dev_srv_ctg>0 and $id_dev_srv_vll>0){$src = 0;}
else{
...
while($dt_mdl = ftc_ass($rq_mdl)){
	...
	if($dt_crc['cnf']>0){$rq_prs = sel_quo("id","dev_prs",array("res","id_jrn"),array("1",$dt_jrn['id']));}
	else{$rq_prs = sel_quo("id","dev_prs","id_jrn",$dt_jrn['id']);}
		while($dt_prs = ftc_ass($rq_prs)){
			...
			while($dt_srv = ftc_ass($rq_srv)){
				if($dt_srv['id_vll']==$id_dev_srv_vll and $dt_srv['ctg']==$id_dev_srv_ctg and $dt_srv['id'] != $id_dev_srv and ($dt_srv['res'] == 0 or $dt_srv['res'] == 6) and $dt_srv['id_frn']==$src){$arr[] = $dt_srv['id'];}
			}
		}
	}
}
